		...
		if (!android_config_driver.interface[i]->disabled && android_config_driver.interface[i]->setup) {
			if (!strcmp(temp_name, android_config_driver.interface[i]->name)) {
				...
				continue;
                                ...
                                if ( strlen(android_config_driver.interface[i]->name) < 127 )
				    strcpy(temp_name,android_config_driver.interface[i]->name);
                                else
                                    strncpy(temp_name,android_config_driver.interface[i]->name,127);
...
void android_enable_function(struct usb_function *f, int enable)
{
	...
	if (dev->cdev)
        {   
            ...
            /*
            ret = usb_gadget_vbus_connect(dev->cdev->gadget);
            if (ret < 0)
            CSY_DBG_ESS("%s vbus connect %d\n", ret);
            */
        ...
        {
            CSY_DBG_ESS("finished setting pid=0x%x but dev->cdev is null ",product_id);
        }    
...
static void samsung_enable_function(int mode)
{
	...
	if(askonstatus == 0xabcd){
		if((mode == USBSTATUS_ASKON)||(mode==USBSTATUS_CHARGING_ONLY))
                {   
                    CSY_DBG_ESS("samsung_enable_function : previous mode is askon or charing only [0x%x] - return \r\n",mode);
                    return;
                }
		inaskonstatus=1;
		askonstatus=0;			
	...
	switch(mode) {
		...
		case USBSTATUS_VTP: /* do not save usb mode */
			...
			break;
#endif
                case USBSTATUS_CHARGING_ONLY: /* Charging only mode : Verizon UI requirement */
			CSY_DBG_ESS("mode = USBSTATUS_CHARGING_ONLY (0x%x)\n", mode);
                        /* Check usb state , usb disconnect and device power off */
	                if (dev->cdev && dev->cdev->gadget &&
			    dev->cdev->gadget->speed != USB_SPEED_UNKNOWN) {
			    CSY_DBG_ESS("Changing only mode : usb device off \n");
                            fsa9480_usb_cb(0);
                        }
			askonstatus=0xabcd;
                        dev->requested_usb_mode = USBSTATUS_CHARGING_ONLY;
		        return; 
                
		case USBSTATUS_ASKON: /* do not save usb mode */
			CSY_DBG_ESS("mode = USBSTATUS_ASKON (0x%x) Don't change usb mode\n", mode);
			askonstatus=0xabcd;
                        dev->requested_usb_mode = USBSTATUS_ASKON;\
		        return;                      
	}


        /* only for askon mode : Vbus check routine */
        /* power consumption bug fix on askon mode */
        if((inaskonstatus == 1) && dev->cdev)
        {
	    if(dev->cdev->gadget)
            {   
                vbus_state=usb_gadget_vbus_detect(dev->cdev->gadget);
                CSY_DBG_ESS("Current Vbus state(%d)\n", vbus_state);
                if (!(vbus_state==1))   
                {
                    /* this mode change does not come form askon action */
                    fsa9480_usb_cb(0);
                }
            }
        } 
...
static ssize_t tethering_switch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value) {
		...
		CSY_DBG_ESS("Disable tethering : previous usb mode = 0x%x\n",a_dev->requested_usb_mode);
                if (!(a_dev->requested_usb_mode == USBSTATUS_CHARGING_ONLY))
                {
		...
		switch(a_dev->requested_usb_mode) {
			...
			case USBSTATUS_ADB:
				return sprintf(buf, "[UsbMenuSel] ACM_ADB_UMS\n");
                        case USBSTATUS_CHARGING_ONLY:
                                return sprintf(buf,"[UsbMenuSel] USBSTATUS_CHARGING_ONLY \n");
		}
	}
	else {
...
{     
	...
	sscanf(buf, "%d", &value);
         CSY_DBG_ESS("UsbMenuSel_switch_store : start (%d / %d)\n", value,prev_value);    
        if ( prev_value == value )
        {
            if (value == 3 ) 
                askonstatus=0xabcd;
            CSY_DBG_ESS("UsbMenuSel_switch_store : error return (%d)\n", value);
            return size;
        }
	...
	switch(value) {
		...
		case 3:
			...
			break;
                case 4:
			CSY_DBG_ESS("Enable Charging only(%d)\n", value);
                        prev_value = value;
			samsung_enable_function(USBSTATUS_CHARGING_ONLY);
			break;                        
		default:
			CSY_DBG("Fail : value(%d) is not invaild.\n", value);
	}
        
        CSY_DBG_ESS("UsbMenuSel_switch_store : Store and return (%d / %d)\n", value,prev_value);    
	return size;
}
#else
static ssize_t UsbMenuSel_switch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
...
static DEVICE_ATTR(UsbMenuSel, S_IRGRP |S_IWGRP | S_IRUSR | S_IWUSR|S_IROTH, UsbMenuSel_switch_show, UsbMenuSel_switch_store);
...
static DEVICE_ATTR(AskOnStatus, S_IRGRP |S_IWGRP | S_IRUSR | S_IWUSR|S_IROTH, AskOnStatus_switch_show, AskOnStatus_switch_store);


/* sysfs for to show status of
 * Path (/sys/devices/platform/android_usb/ChgUsbMode)
 */
static ssize_t ChgUsbMode_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{	
	struct android_dev *a_dev = _android_dev;

	CSY_DBG("chg_usb_mode = %s\n", a_dev->chg_usb_mode);
 ...
 * Path (/sys/devices/platform/android_usb/ChgUsbMode)
 */
static ssize_t ChgUsbMode_switch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{		
	struct android_dev *a_dev = _android_dev;
	
	CSY_DBG("buf : %s\n", buf);
        if ( strlen(buf) < 254 )
	    strcpy(a_dev->chg_usb_mode, buf);
        else
        {
            strncpy(a_dev->chg_usb_mode, buf,254);
            printk(KERN_INFO "ChgUsbMode_switch_store: buf length overrun 254 byte\n");
        }

	//sprintf(a_dev->chg_usb_mode, "%s", buf);

	return size;
}

static DEVICE_ATTR(ChgUsbMode, S_IRGRP |S_IWGRP | S_IRUSR | S_IWUSR|S_IROTH, ChgUsbMode_switch_show, ChgUsbMode_switch_store);
...
/* Create attribute of sysfs as '/sys/devices/platform/android_usb/tethering.
   This file is used only for DataRouter to handle AT command - chage usb mode */
	if (device_create_file(&pdev->dev, &dev_attr_ChgUsbMode) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_ChgUsbMode.attr.name);	
#endif
