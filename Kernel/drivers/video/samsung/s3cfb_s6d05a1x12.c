...
const unsigned short SEQ_POWER_SETTING_SEQ_SMD[] = {
	0xEF, 0x74,
	DATA_ONLY, 0x20,
...
const unsigned short SEQ_INIT_SEQ_SMD[] = {
	0xB4, 0x10,
	DATA_ONLY, 0x00,
	DATA_ONLY, 0x32,
	DATA_ONLY, 0x32,
...
const unsigned short SEQ_GAMMA_SETTING_SMD[] = {
...
static unsigned short SEQ_BACKLIGHT_SMD[] = {
	0x53, 0x2c,
...
const unsigned short SEQ_GAMMA_SETTING_SMD2[] = {
...
const unsigned short SEQ_GAMMA_SETTING_SMD3[] = {
...
static void update_brightness(struct s5p_lcd *lcd)
{
	...
	if (lcd->lcd_type == LcdTypeSHARP){
		SEQ_BRIGHTNESS_SETTING[1]=0X00;
		s6e63m0_panel_send_sequence(SEQ_BRIGHTNESS_SETTING);
		udelay(100); 
		
		bl_level = (bl_level * 185)/255;  //MBjclee
		SEQ_BRIGHTNESS_SETTING[1]=bl_level;
		s6e63m0_panel_send_sequence(SEQ_BRIGHTNESS_SETTING);
	}
	else if (lcd->lcd_type == LcdTypeSMD){
		SEQ_BACKLIGHT_SMD[13]=0X00;
		s6e63m0_panel_send_sequence(SEQ_BACKLIGHT_SMD);
		udelay(100); 
		
		bl_level = (bl_level * 165)/255;  //MBjclee
		SEQ_BACKLIGHT_SMD[13]=bl_level;
		s6e63m0_panel_send_sequence(SEQ_BACKLIGHT_SMD);
	}
	printk("bl:%d ,lcd_type = %d \n", bl_level, lcd->lcd_type);	
...
static void tl2796_ldi_enable(struct s5p_lcd *lcd)
{
	...
	if (lcd->lcd_type == LcdTypeSHARP){
		...
		s6e63m0_panel_send_sequence(SEQ_POWER_SETTING_SEQ_SMD);
		s6e63m0_panel_send_sequence(SEQ_INIT_SEQ_SMD);
		s6e63m0_panel_send_sequence(SEQ_BACKLIGHT_SMD);
		s6e63m0_panel_send_sequence(SEQ_GAMMA_SETTING_SMD3);
	}	
...
static int __devinit tl2796_probe(struct spi_device *spi)
{
	...
	if(HWREV  > 5){
		if(gpio_get_value(GPIO_LCD_TYPE))
			lcd->lcd_type = LcdTypeSHARP;
		else
			lcd->lcd_type = LcdTypeSMD;
	}
	else 
		lcd->lcd_type = LcdTypeSHARP;

	printk("[get_lcd_type]lcd_type = %d , %d \n" ,lcd->lcd_type ,HWREV);
