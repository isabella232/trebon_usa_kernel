...
static unsigned int HWREV=9;
#else
extern unsigned int HWREV;
#endif
#if defined(TIKAL_USA_MPCS)
HWREV = 9
#endif
...
__setup("s3cfb_tl2796.lcd_type=", check_lcdtype);
...
static void update_brightness(struct s5p_lcd *lcd)
{
	...
	if (lcd->on_19gamma) {
		if (lcd_type == LCD_PANEL_SM2) {
			ld9040_panel_send_sequence(lcd, pdata->gamma19_table_sm2[gamma_value]);
		} else if ((lcd_type == LCD_PANEL_M2) || (HWREV >= 8)) {
			ld9040_panel_send_sequence(lcd, pdata->gamma19_table_hw8[gamma_value]);
		} else {
			ld9040_panel_send_sequence(lcd, pdata->gamma19_table[gamma_value]);
		}
	} else {
		if (lcd_type == LCD_PANEL_SM2) {
...
static void ld9040_ldi_enable(struct s5p_lcd *lcd)
{
	...
	if (HWREV >= 8) {
		...
		ld9040_panel_send_sequence(lcd, pdata->seq_display_set);
		ld9040_panel_send_sequence(lcd, pdata->seq_panel_set);
	}
