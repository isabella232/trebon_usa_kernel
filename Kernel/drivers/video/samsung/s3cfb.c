...
#define GETLOGADDR(a)	(((unsigned long)(a)) + (~0x30000000 + 1))
...
extern void s6d16a0x_ldi_disable(void);
	...
	static struct work_struct s3cfb_s6d05a3_lm347_work;
	struct workqueue_struct *work_queue_lm347;
	extern void s6d05a3_lcd_reset(void);
#endif
...
static struct struct_frame_buf_mark  frame_buf_mark = {
	...
	.p_fb   = 0,
#if defined(CONFIG_MACH_VIPER) || ((defined(CONFIG_MACH_CHIEF))&& !(defined (CONFIG_TIKAL_MPCS))) // heatup - chief
	.resX   = 320,
	.resY   = 480,
#else
	.resX   = 480,
...
static int s3cfb_map_video_memory(struct fb_info *fb)
{
	 ...
	 */
   	...
   	frame_buf_mark.p_fb = GETLOGADDR(pdata->pmem_start);
#else
...
#if defined(CONFIG_MACH_TREBON)
extern void trebon_bl_setup(void);
extern void trebon_bl_suspend(void);

void s3cfb_s6d05a3_lm347_workfunc(struct work_struct *work)
{	
	printk("\n*********** Inside %s *********\n",__func__);
	s6d05a3_lcd_reset();
	s6d05a3_ldi_init();
...
static int __devinit s3cfb_probe(struct platform_device *pdev)
{
	...
	s5p_gpio_set_drvstr(GPIO_DISPLAY_PCLK, 0x01);
	s5p_gpio_set_drvstr(GPIO_DISPLAY_VSYNC, 0x01);
	s5p_gpio_set_drvstr(GPIO_DISPLAY_HSYNC, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D0, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D1, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D2, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D3, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D4, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D5, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D6, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D7, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D8, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D9, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D10, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D11, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D12, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D13, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D14, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D15, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D16, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D17, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D18, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D19, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D20, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D21, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D22, 0x01);
	s5p_gpio_set_drvstr(GPIO_LCD_D23, 0x01);	
#endif
...
#if defined(CONFIG_MACH_TREBON)
	work_queue_lm347 = create_singlethread_workqueue("LM347_workqueue");
	if(!work_queue_lm347) {
                ret = -ENOMEM;
	...
	INIT_WORK(&s3cfb_s6d05a3_lm347_work, s3cfb_s6d05a3_lm347_workfunc);
#endif
...
err_irq:
#if defined(CONFIG_MACH_TREBON)
	destroy_workqueue(work_queue_lm347);
#endif
...
#if defined(CONFIG_FB_S3C_S6D16A0X) && defined (CONFIG_MACH_CHIEF) && !(defined (CONFIG_TIKAL_MPCS)) 
void chief_lcd_cfg_gpio_early_suspend(void)
{
		int i;
...
#if defined(CONFIG_MACH_TREBON)
	cancel_work_sync(&s3cfb_s6d05a3_lm347_work);
	 trebon_bl_suspend();
	 s6d05a3_ldi_disable();
...
#elif defined(CONFIG_FB_S3C_S6D16A0X) && defined (CONFIG_MACH_CHIEF) && !(defined (CONFIG_TIKAL_MPCS))
	{
	s6d16a0x_ldi_disable();
	chief_lcd_cfg_gpio_early_suspend();
}
...
void s3cfb_late_resume(struct early_suspend *h)
{
	...
	ret = regulator_enable(fbdev->vlcd); //VDD 1.8V
	if (ret < 0)
		dev_err(fbdev->dev, "failed to enable vlcd\n");

	ret = regulator_enable(fbdev->vcc_lcd);//VCI 3.0V
	if (ret < 0)
		dev_err(fbdev->dev, "failed to enable vcc_lcd\n");
...
#if defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_TIKAL_MPCS)
#if defined(CONFIG_FB_S3C_S6D16A0X)
      /*As per HW team request : Spec says that FIMD clock should be on before I2C communications*/
	if (pdata->reset_lcd)
		pdata->reset_lcd(pdev);
...
#if defined(CONFIG_MACH_TREBON)
	queue_work(work_queue_lm347,&s3cfb_s6d05a3_lm347_work);
#elif defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_TIKAL_MPCS) && !defined(CONFIG_MACH_TREBON)
	s6d16a0x_ldi_init();
	s6d16a0x_ldi_enable();
#endif
	s3cfb_set_vsync_interrupt(fbdev, 1);
