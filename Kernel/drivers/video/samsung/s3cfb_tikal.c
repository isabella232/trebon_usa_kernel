...
#define IRQ_OLED_DET_INT   IRQ_EINT(5)
#else
#define IRQ_OLED_DET_INT (IRQ_EINT_GROUP21_BASE+0) /* J3_0 */
#endif

#define PROBE_STATE 0
#define QUEUE_STATE 1
#define WAIT_STATE 2
#define SUSPEND_STATE 3
#define on 1
#define off 0

extern unsigned int HWREV;
extern ld9040_ldi_init(int on_off);
/*
...
static unsigned int global_oled_enable=0;
...
static void oled_detect_change(struct work_struct *ignored)
{
	struct s3c_platform_fb *pdata = to_fb_plat(fbdev->dev);
	...
	if (global_oled_enable == SUSPEND_STATE)
		return ;

	global_oled_enable= WAIT_STATE;
	...
	ld9040_ldi_init(on);
	global_oled_enable=QUEUE_STATE;
	
	printk("[oled_detect_change]Complete\n"); 
}

static DECLARE_DELAYED_WORK(oled_detect_work, oled_detect_change);

static irqreturn_t oled_det_irq_handler(int irq, struct s3cfb_global *ctrl)
{
	if(global_oled_enable == PROBE_STATE)
		global_oled_enable = QUEUE_STATE;
	else if(global_oled_enable == QUEUE_STATE){
		global_oled_enable = WAIT_STATE;
		schedule_delayed_work(&oled_detect_work, HZ);
		printk("[oled_det_irq_handler]lcd reset\n");
	}
...
#endif	
	/* OLED_DET */
	global_oled_enable = PROBE_STATE;
	s3c_gpio_cfgpin(GPIO_OLED_DET, S3C_GPIO_SFN(0xf));
	s3c_gpio_setpull(GPIO_OLED_DET, S3C_GPIO_PULL_NONE);
	set_irq_type(IRQ_OLED_DET_INT, IRQ_TYPE_EDGE_FALLING);

	ret = request_irq(IRQ_OLED_DET_INT, oled_det_irq_handler,IRQF_DISABLED, "olde_detct", fbdev);

	if (ret < 0) {
	printk(KERN_ERR "GPIO_OLED_DET: Failed to register detect interrupt.\n");
	}
...
void s3cfb_early_suspend(struct early_suspend *h)
{
	...
	printk("%s\n",__FUNCTION__);
	cancel_delayed_work_sync(&oled_detect_work);
	disable_irq(IRQ_OLED_DET_INT);
	global_oled_enable = SUSPEND_STATE;
