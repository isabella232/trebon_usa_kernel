...
int backlight_level = 0;   // disys_forte
    ...
    static struct work_struct bd60910_bl_work;
#endif
...
{
    printk(KERN_INFO "VOB=%d\n", brightness) ;

#if 0
     /* When bd_brightness becomes 1,the below condition causes it to return without calling backlight off sequence*/
	...
	extern void keyled_lcd_off_Ext(); /* keypad_led */ //board_bring
     ...
     /* Brought the above commented code below so that OFF sequence will be executed */
    if (brightness == bd_brightness)
        return ;
...
#if defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_MACH_TREBON)
    cancel_work_sync(&bd60910_bl_work);
#endif    
...
void bd60910_bl_workfunc(struct work_struct *work)
{
