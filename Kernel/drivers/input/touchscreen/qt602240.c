...
static unsigned int gCheckForceCalGood_flag = 0;
...
void qt_Power_Config_Init(void)
{
	if (info_block->info_id.family_id == 0x80) {
		...
		/* Set Active to Idle Timeout to 10 s (one unit = 200ms). */
...
void qt_Acquisition_Config_Init(void)
{
	if (info_block->info_id.family_id == 0x80) {
		...
		acquisition_config_e.chrgtime = 22;
		acquisition_config_e.reserved = 0;
		acquisition_config_e.tchdrift = 5;
		acquisition_config_e.driftst = 2;
...
void qt_Multitouchscreen_Init(void)
{
	if (info_block->info_id.family_id == 0x80) {
		...
		touchscreen_config_e.tchthr = 45;//50;
#else
...
void write_noise_suppression_config_e_TA(void)
{
	...
	noise_suppression_config_e_TA.numtouch = MAX_USING_FINGER_NUM;
...
void qt_Noise_Suppression_Config_Init(void)
{
	...
	if (info_block->info_id.family_id == 0x80) {
...
#if defined(CONFIG_TIKAL_USCC) || defined(CONFIG_TIKAL_MPCS) || defined(CONFIG_MACH_AEGIS)
		noise_suppression_config_e.ctrl = 15;//1;//0
#else
		noise_suppression_config_e.ctrl = 3;//1;//0
#endif
		noise_suppression_config_e.cfg = 132;// 1
		noise_suppression_config_e.calcfg = 114;//64;// 2
		noise_suppression_config_e.basefreq = 24;//0;// 3
		...
		noise_suppression_config_e.mffreq[0] = 1;//10;//0;// 8
		noise_suppression_config_e.mffreq[1] = 2;//20;//0;// 9
		...
		noise_suppression_config_e.gcmaxadcsperx = 64;//48;//100;//

		noise_suppression_config_e.gclimitmin = 4;//

		noise_suppression_config_e.gclimitmax = 64;//48;//64;//
		...
		noise_suppression_config_e.mfinvlddiffthr = 100;//10;//
#elif defined(CONFIG_TIKAL_MPCS)
		noise_suppression_config_e.mfinvlddiffthr = 10;//
#else
		noise_suppression_config_e.mfinvlddiffthr = 10;//10;//
#endif
...
#elif defined(CONFIG_MACH_AEGIS)
		noise_suppression_config_e.mferrorthr = 100;//20;//25
		noise_suppression_config_e.selfreqmax = 5;//1;//
#elif defined(CONFIG_TIKAL_MPCS)
		...
		noise_suppression_config_e.blen = 0;//32;//

		noise_suppression_config_e.tchthr = 25;//50;//35
		noise_suppression_config_e.tchdi = 2;//
		noise_suppression_config_e.movhysti = 3;//
		noise_suppression_config_e.movhystn = 1;//
		noise_suppression_config_e.movfilter = 81;//46;//

		noise_suppression_config_e.numtouch = MAX_USING_FINGER_NUM;
		noise_suppression_config_e.mrghyst = 5;//
		noise_suppression_config_e.mrgthr = 40;//

#if 0 // 20120331 add "if 0"		
...
#else
		noise_suppression_config_e.xloclip = 10;// 
		noise_suppression_config_e.xhiclip = 0;// 
...
#ifdef CONFIG_TIKAL_MPCS
		cte_config_e.actvsyncsperx = 50;//52;//48;//32;//32;//48;//20120217
#else
		cte_config_e.actvsyncsperx = 22;//32;//48;
#endif
		...
		* page byte will sit at 1 until we next send F3 command */
		...
		if ((tch_ch > 0) && (atch_ch == 0)) {  //jwlee change.
			...
			if (qt_timer_state == 1) {
...
#if defined(USE_TS_TA_DETECT_CHANGE_REG) && (defined(CONFIG_TIKAL_USCC) || defined(CONFIG_TIKAL_MPCS) || defined(CONFIG_MACH_AEGIS))
						if (!touch_TA_detect) {
							object_address = get_object_address(PROCG_NOISESUPPRESSION_T48, 0);
							data_byte = 5;
				...
				} else if((qt_time_diff > 30) && (gCheckForceCalGood_flag == 1)) {
					gCheckForceCalGood_flag = 2;
					printk(KERN_DEBUG"[TSP] Checkchip calGood 30ms ForceCalGood_flag = 1\n");

					if((cal_check_flag == 1) && (gCheckForceCalGood_flag == 2))
					{
						u16 obj_address;
						u8 value;
						...
						obj_address = get_object_address(GEN_ACQUISITIONCONFIG_T8, 0);
						
						write_mem(obj_address+6, 1, &acquisition_config_e.atchcalst);
						write_mem(obj_address+7, 1, &acquisition_config_e.atchcalsthr);
						write_mem(obj_address+8, 1, &acquisition_config_e.atchfrccalthr);
						write_mem(obj_address+9, 1, &acquisition_config_e.atchfrccalratio);
				
						if (!touch_TA_detect) {
							obj_address = get_object_address(PROCG_NOISESUPPRESSION_T48, 0);
							value = 5;
							write_mem(obj_address+27, 1, &value);			
						}
						gCheckForceCalGood_flag = 3;
					}					
				} else
					cal_check_flag = 1;
			} else {
		...
		/* cal was bad - must recalibrate and check afterwards */
			...
			if (not_yet_count > 10) {
				not_yet_count = 0;
				printk(KERN_DEBUG"[TSP] decided yet calibration\n");
				calibrate_chip();
			}
		}
	}
...
void NoiseEntryStartFunc(void)
{
	...
	if (object_address == OBJECT_NOT_FOUND) {
		dprintk("[TSP][ERROR] TOUCH_MULTITOUCHSCREEN_T9 object_address : %d\n", __LINE__);
		return;
	}
	...
	if (gResume_flag == 1) {
		...
		data = 90;
#else /*if defined(CONFIG_MACH_AEGIS) //85*/
		data = 95;
#endif
		...
		if (touch_TA_detect == 0) {
			...
			object_address = get_object_address(PROCG_NOISESUPPRESSION_T48, 0);
			if (object_address == OBJECT_NOT_FOUND) {
				dprintk("[TSP][ERROR] PROCG_NOISESUPPRESSION_T48 object_address : %d\n", __LINE__);
				return;
			}		
...
#ifdef CONFIG_TIKAL_MPCS			
			cte_config_e.actvsyncsperx = 50;//52;//48;//20120217
#else
			cte_config_e.actvsyncsperx = 28;
#endif
		}
	}
...
#else
		if(gCheckForceCalGood_flag == 0)
		{

			gCheckForceCalGood_flag = 0xff;
			...
			printk(KERN_DEBUG"[TSP] gCheckForceCalGood_flag Set!!  \r\n");
		}		
#endif		
	}
		...
		if (info_block->info_id.family_id == 0x81) {	/* MXT-224E */
			if ( quantum_msg[0] == 18) {
				if ((quantum_msg[4]&0x5) == 5) {
					...
					if ((!touch_TA_detect) && (noise_median.mferr_setting == false) &&
						(noise_median.median_on_flag == false)) {
						...
						object_address = get_object_address(SPT_CTECONFIG_T46, 0);
						data = 32;
						write_mem(object_address+3, 1, &data);
#endif
...
static void qt602240_early_suspend(struct early_suspend *h)
{
	...
	if (touchscreen_power_state_on == 1) {
		disable_irq(qt602240->client->irq);
		...
		printk(KERN_DEBUG"[TSP] touch already off.\n");
	}
	LEAVE_FUNC;
}
