...
//#include <mach/pmic.h>	Harish_Trebon
...
#include "zinitix_touch_bt4x3_firmware_4key_R04.h"
#else
...
#define GPIO_TOUCH_PIN_NUM	GPIO_TOUCH_INT    //27
...
// Sysfs entries 
char *module_on_status;
char *module_off_status;
char *get_reference_status;
char *get_delta_status;
char *get_scantime_status;
char *channel_count_status;
int16_t raw_data[X_RAW_DATA][Y_RAW_DATA]={{0,},};
int16_t diff_data[X_RAW_DATA][Y_RAW_DATA]={{0,},};
u16	xnode,ynode;
static bool x_ref=0;
static int x_ref_channel;
static bool x_del=0;
static int x_del_channel;
static bool x_scan=0;
static int x_scan_channel;
// Sysfs entries 

extern struct class *sec_class;
struct device *sec_touchscreen_dev, *tsp_dev;
...
#ifdef  CONFIG_TOUCHSCREEN_ZINITIX_4KEY_SUPPORT
u32 BUTTON_MAPPING_KEY[MAX_SUPPORTED_BUTTON_NUM] = {KEY_MENU, KEY_SEARCH, KEY_HOME,KEY_BACK};
#else
...
#if (CONFIG_MACH_TREBON_HWREV == 0x0)
	 s3c_gpio_cfgpin(GPIO_CFG(GPIO_TOUCH_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_direction_output(GPIO_TOUCH_EN, 1);
#else
	 s3c_gpio_cfgpin(GPIO_CFG(78, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
...
bool ts_check_need_upgrade(u16 curVersion, u16 curRegVersion)
{
	...
	/*#if defined(CONFIG_TOUCHSCREEN_ZINITIX_4KEY_SUPPORT)
	if(curRegVersion !=0)  //this patch is to forcefully apply Rev00 version
		return true;
	#endif */
...
retry_init:
	...
	if (i == ZINITIX_INIT_RETRY_CNT || checksum_err) {
		printk(KERN_INFO "fail to check firmware data, checksum_err = %d\r\n", checksum_err);
...
*/		goto fail_init;
	...
	if(chip_firmware_version == 0
		 || chip_firmware_version == 0xffff )
		goto fail_init;
...
fail_init:
	...
	if (++retry_cnt <= ZINITIX_INIT_RETRY_CNT) {
		...
		if ((touch_dev->work_proceedure == TS_IN_RESUME)
			|| (touch_dev->work_proceedure == TS_IN_EALRY_SUSPEND)) {
			printk(KERN_INFO "[zinitix touch] force upgrade skip (proceedure = %d)\r\n", touch_dev->work_proceedure);	 
			return false;
		}
...
extern void keypad_led_onoff_Ext(int OnOff);
...
static ssize_t keypad_brightness_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if (value)
        keypad_led_onoff_Ext(1);
    else
        keypad_led_onoff_Ext(0);
#ifdef DEBUG_LOGCHECK
    printk( " keypad brightness : %d \n", value);
#endif
		...
		for (i = 0; i < touch_dev->cap_info.button_num; i++) {
			if (zinitix_bit_test(touch_dev->icon_event_reg,
				...
				if (BUTTON_MAPPING_KEY[i] == KEY_MENU)
					menu_pressed = 1;
				else if (BUTTON_MAPPING_KEY[i] == KEY_HOME)
					home_pressed = 1;
				else if (BUTTON_MAPPING_KEY[i] == KEY_BACK)
					back_pressed = 1;
				else if (BUTTON_MAPPING_KEY[i] == KEY_SEARCH)
					search_pressed = 1;
			}
		}
...
static void zinitix_early_suspend(struct early_suspend *h)
{
	...
	if(cancel_work_sync(&touch_dev->work)){
	enable_irq(touch_dev->client->irq);
	printk("TSP work pending");
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	module_off_status = "OK";
	return 1;
}

static ssize_t set_module_off_status_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 3, module_off_status);
}
...
static ssize_t set_module_on_status_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 3, module_on_status);
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < misc_touch_dev->cap_info.x_node_num; i++) {
		...
		for (j = 0; j < misc_touch_dev->cap_info.y_node_num; j++) {
			snprintf(buf+written_bytes, 8, "%d\n",
					scantime_data[x_scan_channel][j]);
			written_bytes += 4;
	}
	}
	get_scantime_status = "OK";
	if (written_bytes > 0)
...
static ssize_t scantime_zinitix_store(struct device *dev,
		struct device_attribute *attr, char *buf,size_t size)
{
	sscanf(buf, "%d", &x_scan_channel);
	printk("X channel: %d",x_scan_channel);
	x_scan=1;
	return size;
}

static ssize_t scantime_zinitix_status_show(struct device *dev, struct 
device_attribute *attr, char *buf)
{
	return snprintf(buf, 3, get_scantime_status);
}
#if	USE_TEST_RAW_TH_DATA_MODE
#if 0 
...
static ssize_t zinitix_get_reference_show(struct device *dev, struct 
device_attribute *attr, char *buf)
{
	int i,j;
	int k = 0;

	get_reference_status = "NG";
	...
	for(j = 0 ; j < Y_RAW_DATA; j++)		
	{
		...
		for(i = 0 ; i < X_RAW_DATA ; i++)
		{
			...
			diff_data[i][j] =(int)(((s16)(misc_touch_dev->cur_data[k]-misc_touch_dev->
ref_data[k]))&0xffff);
			k+=Y_RAW_DATA;
		}
	}
	...
	up(&misc_touch_dev->raw_data_lock);	
	get_reference_status = "OK";
	return 1;
	
}
static ssize_t zinitix_get_reference_status_show(struct device *dev, struct 
device_attribute *attr, char *buf)
{
	return snprintf(buf, 3, get_reference_status);
}

static ssize_t zinitix_get_delta_status_show(struct device *dev, struct 
device_attribute *attr, char *buf)
{
	return snprintf(buf, 3, get_reference_status);
}

static ssize_t zinitix_get_reference_result_show(struct device *dev, struct 
device_attribute *attr, char *buf)
{
	int i,j;
	int written_bytes =0;
...
static ssize_t zinitix_get_reference_result_store(struct device *dev, struct 
device_attribute *attr, char *buf, size_t size)
{
	sscanf(buf, "%d", &x_ref_channel);
	printk("X channel: %d",x_ref_channel);
	x_ref=1;
	return size;
}

static ssize_t zinitix_get_delta_result_show(struct device *dev, struct 
device_attribute *attr, char *buf)
{
	int i,j;
	int k = 0;
...
static ssize_t zinitix_get_delta_result_store(struct device *dev, struct 
device_attribute *attr, char *buf, size_t size)
{
	sscanf(buf, "%d", &x_del_channel);
	printk("X channel: %d",x_del_channel);
	x_del=1;
	return size;
}
...
static ssize_t panel_channel_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	//u16	xnode,ynode;
	channel_count_status = "NG";
	xnode =0; 
	ynode =0;
	if (ts_read_data(g_touch_dev->client,
		ZINITIX_TOTAL_NUMBER_OF_X, (u8 *)&xnode, 2) < 0)
			return snprintf(buf, 24, "ts_read_data FAILED!!\n");

	if (ts_read_data(g_touch_dev->client,
		ZINITIX_TOTAL_NUMBER_OF_Y, (u8 *)&ynode, 2) < 0)
			return snprintf(buf, 24, "ts_read_data FAILED!!\n");
			
	channel_count_status = "OK";
	return 1;
	//return snprintf(buf,7, "%d,%d\n",xnode,ynode);
}

static ssize_t panel_channel_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 3, channel_count_status);	
}

static ssize_t panel_channel_result_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf,7, "%d,%d\n",xnode,ynode);
}
...
static ssize_t touchkey_menu_strength(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	short int menukey_sensitivity;
	if (ts_read_data(g_touch_dev->client,
		ZINITIX_MENU_KEY_STRENGTH, (u8 *)&menukey_sensitivity, 2) < 0)
			return snprintf(buf, 24, "ts_read_data FAILED!!\n");
	
	return snprintf(buf, 6, "%d\n", menukey_sensitivity);
}

static ssize_t touchkey_home_strength(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	short int homekey_sensitivity;
	if (ts_read_data(g_touch_dev->client,
		ZINITIX_HOME_KEY_STRENGTH, (u8 *)&homekey_sensitivity, 2) < 0)
			return snprintf(buf, 24, "ts_read_data FAILED!!\n");
	
	return snprintf(buf, 6, "%d\n", homekey_sensitivity);
}

static ssize_t touchkey_search_strength(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	short int searchkey_sensitivity;
	if (ts_read_data(g_touch_dev->client,
		ZINITIX_SEARCH_KEY_STRENGTH, (u8 *)&searchkey_sensitivity, 2) < 0)
			return snprintf(buf, 24, "ts_read_data FAILED!!\n");
	
	return snprintf(buf, 6, "%d\n", searchkey_sensitivity);
}

static ssize_t touchkey_back_strength(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	short int backkey_sensitivity;
	if (ts_read_data(g_touch_dev->client,
		ZINITIX_BACK_KEY_STRENGTH, (u8 *)&backkey_sensitivity, 2) < 0)
			return snprintf(buf, 24, "ts_read_data FAILED!!\n");
	
	return snprintf(buf, 6, "%d\n", backkey_sensitivity);
}

static ssize_t threshold_key_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	short int key_threshold;
	if (ts_read_data(g_touch_dev->client,
		ZINITIX_KEY_THRESHOLD, (u8 *)&key_threshold, 2) < 0)
			return snprintf(buf, 24, "ts_read_data FAILED!!\n");
	
	return snprintf(buf, 6, "%d\n", key_threshold);
}

static ssize_t threshold_screen_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	short int screen_threshold;
	if (ts_read_data(g_touch_dev->client,
		ZINITIX_VIEW_THRESHOLD, (u8 *)&screen_threshold, 2) < 0)
			return snprintf(buf, 24, "ts_read_data FAILED!!\n");
...
//static DEVICE_ATTR(get_touch_test_raw_data, S_IRUGO | S_IWUSR | S_IWGRP, zinitix_get_test_raw_data, zinitix_set_testmode);
...
static DEVICE_ATTR(tsp_firm_update, 0664,
						NULL, firmware_store);
static DEVICE_ATTR(set_all_refer, 0444, rawdata_pass_fail_zinitix, NULL);
static DEVICE_ATTR(tsp_scantime, 0644, scantime_zinitix, NULL);
static DEVICE_ATTR(tsp_scantime_status, 0444, scantime_zinitix_status_show,NULL);
static DEVICE_ATTR(tsp_scantime_result, 0664, scantime_zinitix,scantime_zinitix_store);
...
static DEVICE_ATTR(set_module_off_status, S_IRUGO | S_IWUSR | S_IWGRP,
			set_module_off_status_show, NULL);
static DEVICE_ATTR(set_module_on, S_IRUGO | S_IWUSR | S_IWGRP,
			set_module_on_show, NULL);
static DEVICE_ATTR(set_module_on_status, S_IRUGO | S_IWUSR | S_IWGRP,
			set_module_on_status_show, NULL);
static DEVICE_ATTR(zinitix_get_reference, S_IRUGO | S_IWUSR | S_IWGRP,
			zinitix_get_reference_show, NULL);
static DEVICE_ATTR(zinitix_get_reference_status, S_IRUGO | S_IWUSR | S_IWGRP,
			zinitix_get_reference_status_show, NULL);
static DEVICE_ATTR(zinitix_get_reference_result, S_IRUGO | S_IWUSR | S_IWGRP,
			zinitix_get_reference_result_show, zinitix_get_reference_result_store);
static DEVICE_ATTR(zinitix_get_delta, S_IRUGO | S_IWUSR | S_IWGRP,
			zinitix_get_reference_show, NULL);
static DEVICE_ATTR(zinitix_get_delta_status, S_IRUGO | S_IWUSR | S_IWGRP,
			zinitix_get_reference_status_show, NULL);
static DEVICE_ATTR(zinitix_get_delta_result, S_IRUGO | S_IWUSR | S_IWGRP,
			zinitix_get_delta_result_show, zinitix_get_delta_result_store);
static DEVICE_ATTR(enter_testmode, S_IRUGO | S_IWUSR | S_IWGRP, zinitix_enter_testmode, NULL);
static DEVICE_ATTR(enter_scanmode, S_IRUGO | S_IWUSR | S_IWGRP, zinitix_enter_scanmode, NULL);
static DEVICE_ATTR(enter_normalmode, S_IRUGO | S_IWUSR | S_IWGRP, zinitix_enter_normalmode, NULL);
...
static DEVICE_ATTR(menukey_strength, S_IRUGO, 
			touchkey_menu_strength, NULL);
static DEVICE_ATTR(homekey_strength, S_IRUGO,
			touchkey_home_strength, NULL);
static DEVICE_ATTR(backkey_strength, S_IRUGO,
			touchkey_back_strength, NULL);
static DEVICE_ATTR(searchkey_strength,S_IRUGO,
			touchkey_search_strength, NULL);
static DEVICE_ATTR(key_threshold,S_IRUGO,
			threshold_key_show, NULL);
static DEVICE_ATTR(screen_threshold,S_IRUGO,
			threshold_screen_show, NULL);

static DEVICE_ATTR(tsp_channel, S_IRUGO, panel_channel_show, NULL);
static DEVICE_ATTR(tsp_channel_status, S_IRUGO, panel_channel_status_show, NULL);
static DEVICE_ATTR(tsp_channel_result, S_IRUGO, panel_channel_result_show, NULL);
		...
		const struct i2c_device_id *i2c_id)
{
	...
	struct zinitix_touch_dev *touch_dev;
	int i;
	if (gpio_request(GPIO_TOUCH_EN , "GPIO_GPJ13"))
		zinitix_debug_msg("Failed to request GPIO_TOUCH_EN!");
	ts_power_enable(1);
	...
	gpio_get_value(GPIO_TOUCH_EN) ? "on" : "off");
...
*******/
	...
	s3c_gpio_cfgpin(GPIO_TOUCH_INT, S3C_GPIO_SFN(0xf)); s3c_gpio_setpull(GPIO_TOUCH_INT,S3C_GPIO_PULL_UP);
	...
	if (client->irq) {
		ret = request_irq(client->irq, ts_int_handler,
			IRQF_DISABLED, ZINITIX_DRIVER_NAME, touch_dev);
		if (ret) {
	...
	tsp_dev = device_create(sec_class,
			NULL, 0, NULL, "tsp");
	if (IS_ERR(tsp_dev))
	...
	if (device_create_file(tsp_dev,
			&dev_attr_tsp_scantime) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_tsp_scantime.attr.name);

	if (device_create_file(tsp_dev,
			&dev_attr_tsp_scantime_status) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_tsp_scantime_status.attr.name);

	if (device_create_file(tsp_dev,
			&dev_attr_tsp_scantime_result) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_tsp_scantime_result.attr.name);
	...
	if (device_create_file(tsp_dev,
			    &dev_attr_set_module_off_status) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_set_module_off_status.attr.name);
	...
	if (device_create_file(tsp_dev,
			    &dev_attr_set_module_on_status) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_set_module_on_status.attr.name);
	
	if (device_create_file(tsp_dev,
			    &dev_attr_tsp_channel) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_tsp_channel.attr.name);

	if (device_create_file(tsp_dev,
			    &dev_attr_tsp_channel_status) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_tsp_channel_status.attr.name);
				
	if (device_create_file(tsp_dev,
			    &dev_attr_tsp_channel_result) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_tsp_channel_result.attr.name);
	...
	if (device_create_file(tsp_dev,
			    &dev_attr_zinitix_get_reference) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_zinitix_get_reference.attr.name);
				
	if (device_create_file(tsp_dev,
			    &dev_attr_zinitix_get_reference_status) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_zinitix_get_reference_status.attr.name);
				
	if (device_create_file(tsp_dev,
			    &dev_attr_zinitix_get_reference_result) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_zinitix_get_reference_result.attr.name);
				
	if (device_create_file(tsp_dev,
			    &dev_attr_zinitix_get_delta) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_zinitix_get_delta.attr.name);
				
	if (device_create_file(tsp_dev,
			    &dev_attr_zinitix_get_delta_status) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_zinitix_get_delta_status.attr.name);
				
	if (device_create_file(tsp_dev,
			    &dev_attr_zinitix_get_delta_result) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_zinitix_get_delta_result.attr.name);

	if (device_create_file(sec_touchscreen_dev,
			    &dev_attr_screen_threshold) < 0)
		pr_err("Failed to create device file(%s)!\n",
				dev_attr_screen_threshold.attr.name);
	...
	if (device_create_file(sec_touchkey_dev, &dev_attr_touchkey_search) < 0)
		pr_err("Failed to create device file(%s)!\n",
	...
	if (device_create_file(sec_touchkey_dev, 
	 	&dev_attr_menukey_strength) < 0)
    		pr_err("Failed to create device file(%s)!\n",
				dev_attr_menukey_strength.attr.name); 
	
	if (device_create_file(sec_touchkey_dev, 
	 	&dev_attr_homekey_strength) < 0)
    		pr_err("Failed to create device file(%s)!\n",
				dev_attr_homekey_strength.attr.name); 
	
	if (device_create_file(sec_touchkey_dev, 
	 	&dev_attr_backkey_strength) < 0)
    		pr_err("Failed to create device file(%s)!\n",
				dev_attr_backkey_strength.attr.name); 

	if (device_create_file(sec_touchkey_dev, 
	 	&dev_attr_searchkey_strength) < 0)
    		pr_err("Failed to create device file(%s)!\n",
				dev_attr_searchkey_strength.attr.name); 

	if (device_create_file(sec_touchkey_dev, 
	 	&dev_attr_key_threshold) < 0)
    		pr_err("Failed to create device file(%s)!\n",
...
static int zinitix_touch_remove(struct i2c_client *client)
{
	...
	/*device_remove_file(touch_misc_device.this_device,
		&dev_attr_get_touch_test_raw_data);*/
