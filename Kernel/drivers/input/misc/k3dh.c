...
static const int kr3dh_position_map[][3][3] = {
        ...
        {{-1,  0,  0}, { 0,  1,  0}, { 0,  0, -1}}, /* bottom/lower-left */
...
struct k3dh_data {
	...
	atomic_t position;
#endif
#ifdef USE_K3DH_EARLY_SUSPEND
        struct early_suspend early_suspend;
#endif
};

extern struct class *sec_class;
struct k3dh_data *k3dh_pdata;
int is_suspend;
...
static int k3dh_set_delay(struct k3dh_data *k3dh, s64 delay_ns)
{
	...
	printk("old = %lldns, new = %lldns, odr = 0x%x\n",
		     k3dh_get_delay(k3dh), delay_ns, odr_delay_table[i].odr);
				 ...
				 struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(atomic_read(&k3dh->opened) && (is_suspend != 1))
        hrtimer_cancel(&k3dh->timer);
...
{
	printk("%s\n", __func__);
	if (atomic_read(&k3dh_pdata->opened) > 0) {
		is_suspend = 1;
		k3dh_disable(k3dh_pdata);
	}
}
...
static int k3dh_calibration_data_reset(void)
{
       int err=0;
	struct file *cal_filp = NULL;
	struct k3dh_acc cal_data = {0};
	mm_segment_t old_fs;	
					...
					const char *buf, size_t count)
{
	int err;
        ...
        if ( count > 1 && buf[0] == '0' ) 
        {
            /* For Calibration data Reset App will send value "0" to sys file */
              err = 	k3dh_calibration_data_reset();
              if (err < 0)
		pr_err("%s: k3dh_calibration_data_reset() failed\n", __func__);
...
#ifdef USE_K3DH_EARLY_SUSPEND
	k3dh->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	k3dh->early_suspend.suspend = k3dh_early_suspend;
	k3dh->early_suspend.resume = k3dh_late_resume;
	register_early_suspend(&k3dh->early_suspend);
...
#ifdef USE_K3DH_EARLY_SUSPEND
	unregister_early_suspend(&k3dh->early_suspend);
	k3dh_pdata = NULL;
#endif	  /* CONFIG_HAS_EARLYSUSPEND */
	device_destroy(sec_class, 0);
	device_destroy(k3dh->acc_class, MKDEV(ACC_DEV_MAJOR, 0));
