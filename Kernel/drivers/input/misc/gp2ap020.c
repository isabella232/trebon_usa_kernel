...
int gp2a_i2c_read(struct gp2a_data *gp2a, u8 reg, unsigned char *rbuf ,int len )
{
	int err = 0;
...
static DEVICE_ATTR(lightsensor_file_state, S_IRUGO|S_IWUSR, lightsensor_file_state_show, NULL);
...
int opt_i2c_write(u8 reg, u8 *val, u16 addr, int adapt_num)
{
	...
	printk(KERN_INFO "[%s] : -[%d] ",__func__,err);
	return err;
}
#endif
...
int lightsensor_get_adc(struct gp2a_data *data)
{
	...
	if (1) {  /* GP2AP 030 */
		if (100 * D1_raw_data <= 40 * D0_raw_data) {
			light_alpha = 878;
			light_beta = 0;
		} else if (100 * D1_raw_data <= 51 * D0_raw_data) {
			light_alpha = 2855;
			light_beta = 4864;
...
static int gp2a_opt_probe(struct platform_device *pdev)
{
	...
	*/
...
#if 0
	value = 0x00;
	err = gp2a_i2c_write(gp2a,COMMAND1,&value);
	if (err < 0) {
	...
	sensors_class = class_create(THIS_MODULE, "lightsensor");
...
*/
	if (device_create_file(gp2a->proximity_dev, &dev_attr_lightsensor_file_state) < 0) {
		pr_err("%s: could not create device file(%s)!\n", __func__,
...
static int gp2a_opt_remove(struct platform_device *pdev)
{
	...
	if (sensors_class != NULL) {
		...
		device_remove_file(gp2a->proximity_dev, &dev_attr_lightsensor_file_state);
...
static int proximity_onoff(u8 onoff, struct gp2a_data  *data)
{
	...
	if (onoff) {
		...
		/*if (get_data == 0xC1)
		   return 0; */
		for (i = 0; i < COL; i++) {
			err = gp2a_i2c_write(data,gp2a_original_image[i][0],&gp2a_original_image[i][1]);
			if (err < 0)
		...
		err = gp2a_i2c_write(data,COMMAND2,&value);
			 ...
			 const struct i2c_device_id *id)
{
	...
	return 0;
err_no_device:
	printk(KERN_ERR "GP2A i2c no device error");
	return -1;
}
