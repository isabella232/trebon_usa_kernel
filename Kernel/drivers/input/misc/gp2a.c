...
struct gp2a_data {
	...
	int irq;
#ifdef __HEADSET_NOISE_HW_BUG_WORKAROUND__  //EarJack Issue in VOIP
...
static int proximity_onoff(struct gp2a_data *,u8 );
        ...
        char *buf)
{
    ...
    struct gp2a_data *gp2a = input_get_drvdata(input_data);
	
	pr_info("%s: headset_connected=%d, fake_enable=%d debug_irq_nest=%d\n", __func__, gp2a->headset_connected, gp2a->fake_enable, gp2a->debug_irq_nest);
	
    return sprintf(buf, "%d\n", gp2a->headset_connected);
}
        ...
        size_t count)
{
	...
	gp2a->headset_connected = value;

	pr_info("%s: headset status : %d  power_state %d\n", __func__, gp2a->headset_connected,(gp2a->power_state & PROXIMITY_ENABLED));

	if(gp2a->fake_enable == 1 && gp2a->headset_connected == 0 && !(gp2a->power_state & PROXIMITY_ENABLED)) {
		// headset is disconnected and proximity sensor was enabled
		pr_info("%s: fake_enable -> enable\n", __func__);
		proximity_onoff(gp2a,1);
		gp2a->debug_irq_nest++;
		gp2a->fake_enable = 0;
	} else if(gp2a->headset_connected == 1 && gp2a->fake_enable == 0 && (gp2a->power_state & PROXIMITY_ENABLED)) {
...
#if defined(__HEADSET_NOISE_HW_BUG_WORKAROUND__)
static struct device_attribute dev_attr_headset_connected =
	__ATTR(headset_connected, S_IRUGO | S_IWUSR | S_IWGRP,
...
static int proximity_onoff(struct gp2a_data *gp2a ,u8 onoff)
{
			  ...
			  const struct i2c_device_id *id)
{
	...
	gp2a->val_state = 1; /* Initialization */
