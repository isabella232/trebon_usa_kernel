		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (new_enable) {
		...
		ret = hrtimer_cancel(&k3g_data->timer);

		if(ret <= 0)
			pr_err("%s, hrtimer_cancel ret = %d\n", __func__, ret);
...
static int k3g_remove(struct i2c_client *client)
{
	...
	device_remove_file(&k3g_data->input_dev->dev, &dev_attr_gyro_selftest);
