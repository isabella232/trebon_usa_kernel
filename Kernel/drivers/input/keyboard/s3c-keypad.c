...
static int volume_key_wakeup = 0;
...
/* keypad_led */
int keyled_lcdOn = 0;
static void keyled_timer_start(void);
static void keyled_timer_stop(void);

void keyled_lcd_on_Ext(void){ keyled_lcdOn = 1;  if(keypad_clock) keyled_timer_start(); }
void keyled_lcd_off_Ext(void){ keyled_lcdOn = 0; if(keypad_clock) keyled_timer_stop(); }
...
#if defined(CONFIG_MACH_CHIEF) && (CONFIG_MACH_TREBON)
		case 1:
#else
        case 6 :
#endif
...
#if defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_TIKAL_MPCS)
static DEVICE_ATTR(key_pressed, 0664 /*S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH*/, keyshort_test, NULL);
#endif
...
/*
* All key keysense and keyscan pins are connected to keypad controller. It is same in Tikal_USCC, Tikal_MPCS, StealthV
* During call, IRQ is enabled for Volume_Up key
* Keyled timer allow keypad led to be ON for 1.5sec
*/
...
static int s3c_keypad_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	keyiffc = readl(key_base+S3C_KEYIFFC);
       keyled_timer_stop(); /* keypad_led blinks for 1.5 sec, led on-off controlled by timer*/
	...
	else{/* Normal sleep congiuration */
	    s3c_keypad_port_sleep(KEYPAD_ROWS, KEYPAD_COLUMNS);
	}
	clk_disable(keypad_clock);
	 return 0;
...
#else 	/*below else will compile for models 
		  other than TIKAL_MPCS (SCH_R940)*/
