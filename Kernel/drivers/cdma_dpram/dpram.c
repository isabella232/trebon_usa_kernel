...
void print_rx_fmt_frame(const u8 *buff, u32 size)
{
	format_hdr_type  fmt_hdr;

	fmt_hdr.main_cmd = *(u8 *)(buff + 8);
	fmt_hdr.sub_cmd = *(u8 *)(buff + 9);
	fmt_hdr.cmd_type = *(u8 *)(buff +10);

    	printk("[DPRAM] (M)%s, (S)%s, (T)%s\n",
...
void print_tx_fmt_frame(const u8 *buff, u32 size)
{
	format_hdr_type  fmt_hdr;
...
#define IRQ_DPRAM_INT_N		IRQ_EINT8
...
static int dpram_read_fmt(dpram_device_t *device, const u16 non_cmd)
{
	...
	if (head != tail) {
		...
		if (head > tail) {
			size = head - tail;
#ifdef PRINT_CMD_CHECK
			buff = kmalloc(size, GFP_ATOMIC);
			if (!buff)
				return -ENOMEM;
			memcpy(buff, (u8 *)DPRAM_VBASE + (device->in_buff_addr + tail), size);
			printk("[DPRAM] -------------------------------IPC CMD READ-------------------------------\n");
			print_rx_fmt_frame(buff, size);
#endif
			...
			if(tmp_size>11){
				memcpy(buff, (u8 *)DPRAM_VBASE + (device->in_buff_addr + tail), tmp_size);
				printk("[DPRAM] -------------------------------IPC CMD READ-------------------------------\n");
				print_rx_fmt_frame(buff, size);
			}
			else{
				printk("READ IPC CMD = %02x\n", *(u8 *)(DPRAM_VBASE + (device->in_buff_addr + 8- tmp_size)));
			}
#endif
...
static int dpram_phone_power_on(void)
{
	...
	/* @LDK@ register interrupt handler */
		...
		if (retval < 0) {
			DPRAM_LOG_ERR("[DPRAM] *** dpram failed to register interrupt handler.\n");
			return -1;
		}
	}
...
{
	DPRAM_LOG_WARN("[DPRAM] Phone power Off. - do nothing\n");
}
		...
		const unsigned char *buffer, int count)
{
	...
	if (!device)
		return 0;
#ifdef PRINT_CMD_CHECK
	printk("[DPRAM] -------------------------------IPC CMD WRITE-------------------------------\n");
	print_tx_fmt_frame(buffer, size);
#endif
...
static void command_handler(u16 cmd)
{
	...
	switch (cmd) {
		...
		case INT_MASK_CMD_NORMAL_POWER_OFF:
...
static irqreturn_t phone_active_irq_handler(int irq, void *dev_id)
{
	...
	int ret;
	gpio_state = gpio_get_value(GPIO_PHONE_ACTIVE);
	DPRAM_LOG_INFO("[DPRAM] PHONE_ACTIVE level: %s\n",
			((gpio_state) ? "HIGH" : "LOW "));
...
static int vs_open(struct tty_struct *tty, struct file *filp)
{
	...
	if (dev == NULL) {
		...
		} else if (strcmp(tty->driver->name, "ttyCPLOG") == 0) {
			ret = pdp_activate(&cplog_arg, DEV_TYPE_SERIAL,DEV_FLAG_STICKY);
			if (ret < 0) {
				MULTIPDP_LOG_ERR("failed to create a serial device for ttyCPLOG\n");
				return -ENODEV;
			}
		} else {
...
static int pdp_mux_tty(struct pdp_info *dev, const void *data, size_t len)
{
	...
	while (count) {
		...
		jiffies_to_timeval( jiffies - start_time, &elapsedtime_tv );
 		if (elapsedtime_tv.tv_usec > 10000) {
...
static void init_hw_setting(void)
{
	...
	if (gpio_is_valid(GPIO_CP_RST)) {
		if (gpio_request(GPIO_CP_RST, "dpram/GPIO_CP_RST"))
			printk(KERN_ERR "request fail GPIO_CP_RST\n");
...
static int register_interrupt_handler(void)
{
	...
	retval = request_irq(phone_active_irq, phone_active_irq_handler, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, "VIAActive", NULL);
	if (retval) {
...
static int __devinit dpram_probe(struct platform_device *dev)
{
	...
	printk(KERN_ERR "[DPRAM] *** Entering dpram_probe5()\n");
...
static int dpram_shutdown(struct platform_device *dev)
{
	...
	while (1) {
		if (gpio_get_value(GPIO_PHONE_ACTIVE) || gpio_get_value(GPIO_LTE_ACTIVE)) {
			if (phone_wait_cnt > 5) {
				pr_info("%s: Try to Turn Phone Off, [%d]\n", __func__, phone_wait_cnt);
				gpio_set_value(GPIO_CP_RST, 0);
				gpio_set_value(GPIO_CMC_RST, 0);
				gpio_set_value(GPIO_LTE_PS_HOLD_OFF, 1);
				gpio_set_value(GPIO_VIA_PS_HOLD_OFF, 1);
			...
			if (phone_wait_cnt > 7) {
				pr_crit("%s: PHONE OFF Failed[%d]\n", __func__, phone_wait_cnt);
				break;
			}
			phone_wait_cnt++;
			msleep(1000);
		} else {
			pr_info("%s: PHONE OFF Success[%d]\n", __func__, phone_wait_cnt);
			break;
		}
	}
