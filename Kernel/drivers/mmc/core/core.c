...
retry:		
	#if defined CONFIG_MACH_TREBON
		 if((host->index == 2) && (retry_cnt < 5)){
			printk(KERN_ERR "mmc_rescan : %s , err: %d  retry_cnt= %d \n",mmc_hostname(host), err , retry_cnt);
			retry_cnt++;
			wake_unlock(&mmc_delayed_work_wake_lock);
			mmc_schedule_delayed_work(&host->detect, msecs_to_jiffies(200));
			return;
		 }
	#endif
