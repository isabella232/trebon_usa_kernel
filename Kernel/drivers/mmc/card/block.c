			...
			if (brq.data.blocks > 1 && rq_data_dir(req) == READ) {
				...
				if(mq->rx_retries == 4) {
					...
					printk(KERN_WARNING "%s: retrying using single "
							"block read\n"
							"CMD%d%s : sector %u, nr %u.\n"
							"cmd.error=%d, data.error=%d, stop.error=%d, ",
							req->rq_disk->disk_name,
							brq.cmd.opcode,
							(brq.mrq.stop) ? "(with CMD12)" : " ",
					...
					printk(KERN_WARNING "%s: adjust configuration "
							"and retrying block read(%d/3)\n"
							"CMD%d%s : sector %u, nr %u.\n"
							"cmd.error=%d, data.error=%d, stop.error=%d, ",
							req->rq_disk->disk_name,
							mq->rx_retries,
...
#if 1	/* If READ fail on Sector 0, end all requests */ 
			if (disable_multi &&
					(0 == (unsigned)blk_rq_pos(req)) &&
