...
static void max17040_adjust_fullsoc(struct i2c_client *client)
{
	...
	if (chip->raw_soc > chip->pdata->psoc_full - COMP_FULL_MARGIN)
		chip->full_soc = 
			(chip->raw_soc > 10000) ? 10000 : chip->raw_soc;
	else
		chip->full_soc = chip->pdata->psoc_full -
			COMP_FULL_MARGIN;
	
	chip->full_soc =
		((chip->full_soc - chip->pdata->psoc_empty) * 99 / 100) +
		chip->pdata->psoc_empty;
			    ...
			    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_RCOMP:
		i2c_smbus_write_word_data(chip->client, MAX17040_RCOMP_MSB,
			swab16(val->intval));
		msb = max17040_read_reg(chip->client, MAX17040_RCOMP_MSB);
		lsb = max17040_read_reg(chip->client, MAX17040_RCOMP_LSB);
		printk("%s: rcomp = 0x%x 0x%x\n", __func__, msb, lsb);
		break;
#if defined(CONFIG_AEGIS_USCC)
	case POWER_SUPPLY_PROP_STATUS:
...
static void max17040_get_vcell(struct i2c_client *client)
{
	...
	if (msb < 0 || lsb < 0)
		chip->vcell = min(chip->vcell, 3600000);	// error handling (i2c fail when battery low)
	else
...
static void max17040_get_soc(struct i2c_client *client)
{
	...
	if (socValue >= 0)
		soc   = ((((socValue*10)-6)/10) * 100)/( ( (103*10) - 6)/10);
	else
		soc = 0;
#else
	pure_soc = msb * 100 + (lsb * 100) / 256;
	chip->raw_soc = pure_soc;
	...
	if (msb < 0 || lsb < 0)
		soc = chip->soc;	// error handling (i2c fail when battery low)
...
/* Commented for TREBON MAX17043-New Model gauge implementation */
#ifndef CONFIG_MACH_TREBON
	if (chip->pdata)
		i2c_smbus_write_word_data(client, MAX17040_RCOMP_MSB,
					  swab16(chip->pdata->rcomp_value));
	pr_info("%s: RCOMP = 0x%x\n", __func__, chip->pdata->rcomp_value);
#endif 
