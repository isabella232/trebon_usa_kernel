 ...
 **			Reference: Via6419 Internal DPRAM driver 
 **                     (../internal_dpram/dpram.c/.h)
 *
...
/*	S5PV210 Internal Dpram Special Function Register 	*/
...
static int _dpram_ex_init(struct idpramctl *idpram)
{
	...
	/**/dpram_clear();
	printk(KERN_ERR "idpram clear add\n");
	idpram_gpio_init();
...
static int dpram_shared_bank_remap(void)
{
	...
	printk("[iDPRAM] ioremap returned %p\n", dpram_base);
	return 0;
}
...
void dpram_clear(void)
{
	...
	printk("[iDPRAM] *** leaving dpram_clear()\n");
}
...
{
	printk("[iDPRAM] %s %d \n",__FUNCTION__,__LINE__);
	printk("[iDPRAM] Phone power Off. - do nothing\n");
}
...
{
	//printk("[iDPRAM] Entering dpram_irq_handler()\n");
