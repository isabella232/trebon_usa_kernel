/* dpram_chief.c
 *
 * SAMSUNG IDPRAM Device Driver for CDMA Modem
 *
...
#include "dpram_chief.h"

#define S5PV210_GPJ2_0_MSM_DATA_0	(0x2 << 0)
#define S5PV210_GPJ2_1_MSM_DATA_1	(0x2 << 4)
#define S5PV210_GPJ2_2_MSM_DATA_2	(0x2 << 8)
#define S5PV210_GPJ2_3_MSM_DATA_3	(0x2 << 12)
#define S5PV210_GPJ2_4_MSM_DATA_4	(0x2 << 16)
#define S5PV210_GPJ2_5_MSM_DATA_5	(0x2 << 20)
#define S5PV210_GPJ2_6_MSM_DATA_6	(0x2 << 24)
#define S5PV210_GPJ2_7_MSM_DATA_7	(0x2 << 28)

#define S5PV210_GPJ3_0_MSM_DATA_8	(0x2 << 0)
#define S5PV210_GPJ3_1_MSM_DATA_9	(0x2 << 4)
#define S5PV210_GPJ3_2_MSM_DATA_10	(0x2 << 8)
#define S5PV210_GPJ3_3_MSM_DATA_11	(0x2 << 12)
#define S5PV210_GPJ3_4_MSM_DATA_12	(0x2 << 16)
#define S5PV210_GPJ3_5_MSM_DATA_13	(0x2 << 20)
#define S5PV210_GPJ3_6_MSM_DATA_14	(0x2 << 24)
#define S5PV210_GPJ3_7_MSM_DATA_15	(0x2 << 28)

#define S5PV210_GPJ4_0_MSM_CSn		(0x2 << 0)
#define S5PV210_GPJ4_1_MSM_WEn		(0x2 << 4)
#define S5PV210_GPJ4_2_MSM_Rn		(0x2 << 8)
#define S5PV210_GPJ4_3_MSM_IRQn		(0x2 << 12)
#define S5PV210_GPJ4_4_MSM_ADVN		(0x2 << 16)

#define S5PV210_GPIOREG(x)		(S5P_VA_GPIO + (x))
#define S5PV210_GPJ4DAT			(S5PV210_GPJ4_BASE + 0x04)
#define S5PV210_GPJ4_OUTPUT(__gpio)	(0x1 << ((__gpio) * 4))
...
#endif
#define IRQ_DPRAM_AP_INT_N  IRQ_MSM     // IDPRAM's special interrupt in AP
...
// temp. block // extern void usb_switch_mode(int);
...
static int dpram_write(dpram_device_t *device, const unsigned char *buf, int len)
{
    ...
    if (g_phone_sync != 1 || access == 0 || magic != 0xAA) {
	...
	if (len > free_space) {
		LOGE("No space: len %d vs. free %d head %d tail %d\n", len, free_space, head, tail);
		irq_mask = INT_MASK_VALID | device->mask_req_ack;
		dpram_send_mbx_BA(irq_mask);
		return -EAGAIN;
	}

#ifdef PRINT_DPRAM_WRITE
	LOGA("len %d vs. free %d head %d tail %d\n", len, free_space, head, tail);
    dpram_print_packet(buf, len);
#endif

	/* +++++++++ head ---------- tail ++++++++++ */
	if (head < tail) {
		dest = (u8 *)(DPRAM_VBASE + device->out_buff_addr + head);
	...
	/* ------ tail +++++++++++ head ------------ */
		...
		dest = (u8 *)(DPRAM_VBASE + device->out_buff_addr + head);
		memcpy_toio(dest, buf, (len > last_size ? last_size : len));
		if (len > last_size) {
			dest = (u8 *)(DPRAM_VBASE + device->out_buff_addr);
			memcpy_toio(dest, (buf + last_size), (len - last_size));
		}
	}

	/* Update new head */
	up_head = (u16)((head + len) % device->out_buff_size);
	iowrite16(up_head, (void *)(DPRAM_VBASE + device->out_head_addr));
        ...
        {            
            ...
            if (dev->vs_dev.tty != NULL && dev->vs_dev.refcount)
            {
                ...
                else
                {
                    ...
                    tty_flip_buffer_push(dev->vs_dev.tty);
#if 0
                    LOGA("RAW pre_data_size: %d, len-pre_data_size: %d, ret: %d\n", pre_data_size, len- pre_data_size, ret);
#endif
                }
            }
            else
...
static void dpram_phone_power_on(void)
{
...
#if 1 // for QSC6085 JTAG debugging
    ...
    LOGA("QSC_PHONE_ON: %s, QSC_RESET_N: %s, PHONE_ACTIVE: %s\n",
...
static int dpram_init_magic_num(void)
{
    ...
    LOGA("Read_MagicNum : 0x%04x ", ret_value);
...
static int kernel_sec_dump_cp_handle2(void)
{
    ...
    __raw_writel
    (
        (S5PV210_GPJ4_0_MSM_CSn | S5PV210_GPJ4_1_MSM_WEn |
         S5PV210_GPJ4_2_MSM_Rn  | S5PV210_GPJ4_OUTPUT(3) |
         S5PV210_GPJ4_4_MSM_ADVN),
         S5PV210_GPJ4_BASE
    );
...
static int register_dpram_driver(void)
{
    ...
    if (retval) {
        LOGE("tty_register_driver error : %d \n", retval);
 ...
 */
static void dpram_init_hw(void)
{
    PRINT_FUNC();
    ...
    s3c_gpio_cfgpin(GPIO_C111_DPRAM_INT_N, S3C_GPIO_SFN(0xFF));   
    s3c_gpio_setpull(GPIO_C111_DPRAM_INT_N, S3C_GPIO_PULL_NONE);   
    set_irq_type(IRQ_QSC_INT, IRQ_TYPE_EDGE_RISING);

    s3c_gpio_setpull(GPIO_QSC_ACTIVE, S3C_GPIO_PULL_NONE);
//    set_irq_type(IRQ_QSC_ACTIVE, IRQ_TYPE_EDGE_BOTH);
    set_irq_type(IRQ_QSC_ACTIVE, IRQ_TYPE_EDGE_FALLING);
    ...
    __raw_writel
    ( 
        (S5PV210_GPJ2_0_MSM_DATA_0 | S5PV210_GPJ2_1_MSM_DATA_1 |
         S5PV210_GPJ2_2_MSM_DATA_2 | S5PV210_GPJ2_3_MSM_DATA_3 |
         S5PV210_GPJ2_4_MSM_DATA_4 | S5PV210_GPJ2_5_MSM_DATA_5 |
    ...
    __raw_writel
    (
        (S5PV210_GPJ3_0_MSM_DATA_8  | S5PV210_GPJ3_1_MSM_DATA_9  |
         S5PV210_GPJ3_2_MSM_DATA_10 | S5PV210_GPJ3_3_MSM_DATA_11 | 
         S5PV210_GPJ3_4_MSM_DATA_12 | S5PV210_GPJ3_5_MSM_DATA_13 |
    ...
    (
        (S5PV210_GPJ4_0_MSM_CSn | S5PV210_GPJ4_1_MSM_WEn  | 
         S5PV210_GPJ4_2_MSM_Rn	| S5PV210_GPJ4_3_MSM_IRQn | 
         S5PV210_GPJ4_4_MSM_ADVN),
        S5PV210_GPJ4_BASE
    );
    ...
    gpio_request(GPIO_QSC_PHONE_ON, "QSC PHONE ON");
    gpio_direction_output(GPIO_QSC_PHONE_ON, GPIO_LEVEL_LOW);
    gpio_request(GPIO_QSC_RESET_N, "QSC Reset");
    gpio_direction_output(GPIO_QSC_RESET_N, GPIO_LEVEL_LOW);
}
...
static int register_interrupt_handler(void)
{
    unsigned int dpram_irq, dpram_wakeup_irq, phone_active_irq;
    int retval = 0;
    
    dpram_irq = IRQ_DPRAM_AP_INT_N;
    dpram_wakeup_irq = IRQ_QSC_INT;
    ...
    enable_irq_wake(dpram_wakeup_irq) ; /* Wakeup Source @DISYS */
...
static void check_miss_interrupt(void)
{
    ...
    if ( gpio_get_value(GPIO_QSC_ACTIVE) && !gpio_get_value(IRQ_DPRAM_AP_INT_N) )
    {
        local_irq_save(flags);
