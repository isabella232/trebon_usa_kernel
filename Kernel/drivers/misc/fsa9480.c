...
#if defined(CONFIG_USB_SWITCH_FSA9485_RAW_MODE)
#define DEV1_RAW_MODE_MASK	(DEV_UART)
#endif
...
#if defined(CONFIG_USB_SWITCH_FSA9485_RAW_MODE)
#define DEV2_RAW_MODE_MASK	(DEV_AV | DEV_TTY | DEV_PPD)
#endif
...
/* Timing table for Timing Set 1 register at FSA9485 */
#define REG_TIMING1_VALUE_FSA9485	(ADC_DETECTION_TIME_50MS)
				    ...
				    const char *buf, size_t count)
{
...
#if defined(CONFIG_USB_SWITCH_FSA9485_RAW_MODE)
	if (usbsw->device_id == 0x00) {
		if (((usbsw->dev1 & DEV1_RAW_MODE_MASK) != 0) ||
			((usbsw->dev2 & DEV2_RAW_MODE_MASK) != 0)) {
			/* Accessory Attached Rid type or accessory */
			/* RAW data bit enabled check control register */
			if ((value & ~(CON_SWITCH_OPEN | CON_MANUAL_SW)) != (CON_WAIT))
				return 0;
		} else {
			/* Accessory Rid type or accessory is not connected*/
			/* RAW data bit disabled check control register*/
	...
	if (usbsw->device_id == 0x00) {
		/* Support FSA9485 */
		mansw1_value = mansw1_value & ~(0x03);	// clear vbus switching
		mansw1_value |= 0x03;	// set vbus switching bits to "11"
	}
...
void fsa9480_manual_switching(int path)
{
...
#if defined(CONFIG_USB_SWITCH_FSA9485_RAW_MODE)
	if (local_usbsw->device_id == 0x00) {
		if (((local_usbsw->dev1 & DEV1_RAW_MODE_MASK) != 0) ||
			((local_usbsw->dev2 & DEV2_RAW_MODE_MASK) != 0)) {
			/* Accessory Attached Rid type or accessory */
			/* RAW data bit enabled check control register */
			if ((value & ~(CON_SWITCH_OPEN | CON_MANUAL_SW)) != (CON_WAIT))
				return;
	...
	if (path == SWITCH_PORT_VAUDIO) {
		...
		dev_err(&client->dev, "%s: wrong path (%d)\n", __func__, path);
		return;
	}
...
{
	if ((usbsw->device_id != 0x00) && (usbsw->intr & INT_OVP_EN))
		return 0;
...
#if defined(CONFIG_USB_SWITCH_FSA9485_RAW_MODE)
static void fsa9480_raw_change(struct i2c_client *client, bool enable)
{
	unsigned int ctrl_from, ctrl_to;
	int ret;

	ctrl_from = i2c_smbus_read_byte_data(client, FSA9480_REG_CTRL);
	if (ctrl_from < 0)
		dev_err(&client->dev, "%s: err 0x%x\n", __func__, ctrl_from);

	ctrl_to = ctrl_from;

	if (ctrl_from & CON_RAW_DATA) {
		/* RAW data bit disabled */
		if (enable) {
			/* enable RAW data bit */
			ctrl_to = ctrl_from & ~CON_RAW_DATA;
		}
	} else {
		/* RAW data bit enabled */
		if (!enable) {
			/* disable RAW data bit */
			ctrl_to = ctrl_from | CON_RAW_DATA;
		}
	}
	dev_info(&client->dev, "%s: %s RAW data bit CTRL from 0x%x to 0x%x\n",
		__func__, (enable ? "Enable" : "Disable"), ctrl_from, ctrl_to);

	ret = i2c_smbus_write_byte_data(client, FSA9480_REG_CTRL, ctrl_to);
	if (ret < 0)
		dev_err(&client->dev, "%s: err %d\n", __func__, ret);
}
#endif
...
static void fsa9480_detect_dev(struct fsa9480_usbsw *usbsw)
{
	...
	if (val1 || val2) {
		...
		if (pdata->deskdock_cb)
			...
			if (usbsw->device_id == 0x00)
				value |= 0x03;	// FSA9485: set vbus switching bits to "11"
		...
		/* Desk Dock (Stealth-V, Aegis: Car Dock) */
...
#if defined (CONFIG_STEALTHV_USA) || defined (CONFIG_MACH_AEGIS) || defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_MACH_TREBON)
			if (pdata->cardock_cb)
			...
			/* Set manual switching */
#if defined(CONFIG_USB_SWITCH_FSA9485_RAW_MODE)
			value = SW_AUDIO & ~(0x03);	// vbus manual switching
#else
			value = SW_VAUDIO & ~(0x03); // vbus manual switching
#endif
			if (usbsw->device_id == 0x00)
				value |= 0x03;	// FSA9485: set vbus switching bits to "11"

			dev_info(&client->dev, "DEV_AV(ATTACH) -> manual sw1 write 0x%x\n", value);
			ret = i2c_smbus_write_byte_data(client, FSA9480_REG_MANSW1, value);
			if (ret < 0)
				dev_err(&client->dev, "%s: err %d\n", __func__, ret);
...
static void fsa9480_reg_init(struct fsa9480_usbsw *usbsw)
{
	...
	if (usbsw->device_id == 0) {
		/* FSA9485's REG_TIMING1_VALUE_FSA9485 is equal to FSA9480's  REG_TIMING1_VALUE*/
		ret = i2c_smbus_write_byte_data(client, FSA9480_REG_TIMING1,
			REG_TIMING1_VALUE_FSA9485);
	} else {
...
static irqreturn_t fsa9480_irq_thread(int irq, void *data)
{
	...
	if (usbsw->device_id == 0x00) {
		if (((val1 & DEV1_RAW_MODE_MASK) != 0) ||
			((val2 & DEV2_RAW_MODE_MASK) != 0)) {
			/* Accessory Attached Rid type or accessory */
			/* RAW data bit enable */
			fsa9480_raw_change(client, true);
		} else {
			/* Accessory Detached Rid type or accessory */
			/* RAW data bit disable */
			fsa9480_raw_change(client, false);
		}
	}
#endif
			 ...
			 const struct i2c_device_id *id)
{
	...
	/* print device id */
	do {
		usbsw->device_id = i2c_smbus_read_byte_data(client,
			FSA9480_REG_DEVID);
	} while (usbsw->device_id < 0);
	dev_info(&client->dev, "%s: Device ID = 0x%x\n",
		__func__, usbsw->device_id);
...
static void fsa9480_shutdown(struct i2c_client *client)
{
	...
	if (!usbsw) {
		dev_err(&client->dev, "%s: usbsw is NULL!\n", __func__);
		return;
	}

	dev_info(&client->dev, "%s: RAW mode Disable & MASK intterrupt & Enable Auto switching\n", __func__);
	ctrl |= CON_INT_MASK;

	ret = i2c_smbus_write_byte_data(client, FSA9480_REG_CTRL, ctrl);
	if (ret < 0)
		dev_err(&client->dev, "%s: CTRL register write err(%d)\n", ret);
}
