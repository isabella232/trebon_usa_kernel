...
#define VIBRATOR_PERIOD	38110	// 1 / (128 * 205 Hz) in nanoseconds
#define VIBRATOR_DUTY	37729	// 0.99 * VIBRATOR_PERIOD
#else
...
static struct work_struct timed_output_vibwork;
#endif

#if defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_TIKAL_MPCS)
struct regulator *vibrator_regulator ; /* LDO12 */
#endif

#if defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_TIKAL_MPCS)
void timed_output_vibdisable(struct work_struct *work)
{
	printk("[timed_output_vibdisable] DISABLE\n");
	...
	if(!timeout) {
...
#if defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_TIKAL_MPCS)
		schedule_work(&timed_output_vibwork);
#endif		
		#if !defined(CONFIG_MACH_CHIEF)
		...
		printk("[VIBETONZ] ENABLE\n");
            	...
            	if (regulator_enable(vibrator_regulator))
            		pr_err("Failed to enable regulator vibrator\n");
        #endif
...
#if defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_TIKAL_MPCS)
    /*ldo 12*/
	if (IS_ERR_OR_NULL(vibrator_regulator)) {
		vibrator_regulator = regulator_get(NULL, "vibrator");
		if (IS_ERR_OR_NULL(vibrator_regulator)) {
			pr_err("failed to get vibrator regulator");
			return -EINVAL;
		}
		INIT_WORK(&timed_output_vibwork, timed_output_vibdisable);
	}
#endif
...
#if defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_TIKAL_MPCS)
	if (regulator_disable(vibrator_regulator))
		pr_err("Failed to disable regulator vibrator\n");
#endif
}
