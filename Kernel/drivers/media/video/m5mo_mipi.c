...
enum m5moLS_frame_size 
{
    ...
    M5MO_LS_CAPTURE_SXGA, /* 1280 x 960 */
    M5MO_LS_CAPTURE_W1_5MP, /* 1600 x 960 */
    M5MO_LS_CAPTURE_2MP, /* 1600 x 1200 */
    M5MO_LS_CAPTURE_W2_4M, /* 2048 x 1232 */
    M5MO_LS_CAPTURE_3M, /* 2048 x 1536 */
...
static struct m5moLS_enum_framesize m5moLS_framesize_list[] = 
{
    ...
    { M5MO_LS_OPRMODE_IMAGE, M5MO_LS_CAPTURE_SXGA,   	    1280,  960 },
    { M5MO_LS_OPRMODE_IMAGE, M5MO_LS_CAPTURE_W1_5MP,   	    1600,  960 },
    { M5MO_LS_OPRMODE_IMAGE, M5MO_LS_CAPTURE_2MP,           1600, 1200 },
    { M5MO_LS_OPRMODE_IMAGE, M5MO_LS_CAPTURE_W2_4M,		    2048, 1232 },
    { M5MO_LS_OPRMODE_IMAGE, M5MO_LS_CAPTURE_3M,		    2048, 1536 },
...
static int 
m5moLS_write_mem(struct i2c_client *client, u8 cmd, u16 data_length, u32 reg, u8* val)
{
	struct i2c_msg msg[1];
	u8 *data;
	int retry = 0, err;
...
static int m5moLS_get_version_in_flash(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	/* Set M-5MoLS Pins */
	m5moLS_write_mem(client, 0x04, 0x0001, (u32)0x50000308, &pin1);
...
static int m5moLS_set_preview_resolution(struct v4l2_subdev *sd)
{
    ...
    if(state->m_movie_mode == 0x00) // Set fps only in camera mode.
    {
        m5moLS_err(&client->dev,"fixed fps %d\n",state->fps);
        m5moLS_write_1byte(M5MO_CATEGORY_PARM, M5MO_PARM_FLEX_FPS, state->fps);
    }
...
static int m5moLS_set_capture_resolution(struct v4l2_subdev *sd)
{
    ...
    switch(index)
    {
        ...
        case M5MO_LS_CAPTURE_SXGA: /* 1280x960 */
        {
            capture_resolution = M5MO_LS_SHOT_1280_960;
            postview_resolution = 0x0B;            
            break;
        }
        ...
        case M5MO_LS_CAPTURE_W2_4M: // 2048 x 1232
        {
            capture_resolution = M5MO_LS_SHOT_2048_1232;
            postview_resolution = 0x0C;            
            break;
        }

        case M5MO_LS_CAPTURE_3M: // 2048 x 1536
        {
            capture_resolution = M5MO_LS_SHOT_2048_1536;
            postview_resolution = 0x0B;            
            break;
        }
...
static int m5moLS_set_af_softlanding(struct v4l2_subdev *sd)
{
    ...
    err = m5moLS_set_mode(sd, M5MO_LS_MONITOR_MODE);
    if(err)
    {
        m5moLS_err(&client->dev, "Can not set operation mode\n");
        return err;
	...
	m5moLS_write_1byte(M5MO_CATEGORY_LENS, M5MO_LENS_AF_MODE, 0x07);
	...
	for(i = 0; i < M5MO_LS_I2C_VERIFY_RETRY; i++) 
	{
		...
		m5moLS_read_1byte(M5MO_CATEGORY_LENS, M5MO_LENS_AF_STATUS, &status);
    ...
    m5moLS_err(&client->dev, "AF soft landing took %d ms : %d\n", (i+1)*10, status);
    
	if((status & 0x01) != 0x00)
	{
	    m5moLS_err(&client->dev, "AF soft landing failed : %d\n", status);
    }    
...
static int m5moLS_set_wdr(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
    ...
    switch(ctrl->value)
    {
        ...
        case WDR_ON:
        {
            ...
            err = m5moLS_write_1byte(M5MO_CATEGORY_CAPPARM, M5MO_CAPPARM_WDR_EN, 0x01);
            break;
        }
...
{   
    ...
    switch(ctrl->value)
    {
        ...
        {
            err = m5moLS_write_1byte(M5MO_CATEGORY_LENS, M5MO_LENS_AF_START, 0x00);
            break;
        }
        
        case M5MO_LS_AF_CONTINUOUS_ON:
        {
            err = m5moLS_write_1byte(M5MO_CATEGORY_LENS, M5MO_LENS_AF_START, 0x02);
            break;
        }
...
static int m5moLS_set_touch_auto_focus(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
    ...
    if(ctrl->value)     /* Start */
    {
        ...
        // Touch AF mode
        if(state->userset.lens.focus_mode == V4L2_FOCUS_MODE_MACRO)
        {
            m5moLS_write_1byte(M5MO_CATEGORY_LENS, M5MO_LENS_AF_MODE, 0x01);
        }
        else
        {
            m5moLS_write_1byte(M5MO_CATEGORY_LENS, M5MO_LENS_AF_MODE, 0x04);
        }
        ...
        state->userset.lens.touch_af_activated = true;
    }
    else
    {
        state->userset.lens.touch_af_activated = false;
    }

    return 0;
}

static int m5moLS_set_focus_mode(struct v4l2_subdev *sd, int focus_mode)
{
    struct m5moLS_state *state = to_state(sd);
    struct i2c_client *client = v4l2_get_subdevdata(sd); 
    int err = -1;

    m5moLS_msg(&client->dev, "%s: Enter :::: Focus Mode = %d(%d)\n", __func__, focus_mode, state->m_focus_mode);
    ...
    switch(focus_mode)
    {
        case FOCUS_MODE_AUTO:
        {
            ...
            state->userset.lens.focus_mode = V4L2_FOCUS_MODE_AUTO;
            break;
        }
        ...
        case FOCUS_MODE_MACRO: 
        {
            ...
            state->userset.lens.focus_mode = V4L2_FOCUS_MODE_MACRO;
            break;
        }
        ...
        {
            err = m5moLS_write_1byte(M5MO_CATEGORY_LENS, M5MO_LENS_AF_MODE, 0x02);
    ...
    if((focus_mode != state->m_focus_mode) && (focus_mode+3 != state->m_focus_mode))
    {
        /* AF initial position */
        m5moLS_write_1byte(M5MO_CATEGORY_LENS, M5MO_LENS_AF_INIT_POS, 0x01);           
    }
    
    state->m_focus_mode = focus_mode;
...
static int m5moLS_set_face_beauty(struct v4l2_subdev *sd)
{
    ...
    {
        m5moLS_info(&client->dev, "%s: Enter : face beauty enabled\n", __func__);
...
static void m5moLS_start_auto_focus(struct v4l2_subdev *sd)
{
    ...
    if(state->userset.lens.focus_mode == V4L2_FOCUS_MODE_MACRO)
    {
        if(state->userset.lens.touch_af_activated)
        {
            af_mode_sel = 0x04;
        }
        else
    ...
    m5moLS_err(&client->dev,"Current AF mode sel : %d\n", af_mode_sel);
    
    m5moLS_write_1byte(M5MO_CATEGORY_LENS, M5MO_LENS_AF_MODE_SEL, af_mode_sel);
...
static void m5moLS_init_parameters(struct v4l2_subdev *sd)
{
    ...
    //state->fps = 0; /* Default value : Variable fps */
    ...
    // state->strm.parm.capture.capturemode = V4L2_MODE_PREVIEW;
...
static int m5moLS_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	// Skip the cases of Starting and Stopping recording
	...
	if (enable) 
	{
		...
		{		
			...
			m5moLS_write_1byte(M5MO_CATEGORY_CAPCTRL, M5MO_CAPCTRL_FRM_SEL, 0x01);
...
static int m5moLS_set_framesize_index(struct v4l2_subdev *sd, unsigned int index)
{
	...
	for(i = 0; i < (sizeof(m5moLS_framesize_list)/sizeof(struct m5moLS_enum_framesize)); i++)
	{
		...
		{
		    framesize_w = m5moLS_framesize_list[i].width;
		    framesize_h = m5moLS_framesize_list[i].height;
		    
            m5moLS_info(&client->dev, "Res : Requested[%dx%d], Selected[%dx%d]\n", 
                        state->pix.width, state->pix.height, framesize_w, framesize_h);
            
			state->framesize_index = m5moLS_framesize_list[i].index;	
			state->pix.width = framesize_w;
			state->pix.height = framesize_h;
...
static int m5moLS_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
    ...
    m5moLS_info(&client->dev, "%s :[ID=%d, Value=%d]\n", __func__, (ctrl->id - offset), ctrl->value);
    ...
    switch (ctrl->id) 
    {
        ...
        {
            err = m5moLS_set_focus_mode(sd, ctrl->value);
            break;
        }

        case V4L2_CID_CAMERA_DEFAULT_FOCUS_POSITION:
        {
            err = m5moLS_set_focus_mode(sd, state->m_focus_mode);
            break;
        }
    	...
    	case V4L2_CID_CAMERA_AF_SOFTLANDING:
    	{
    		err = m5moLS_set_af_softlanding(sd);
    		break;		
        }
...
static int m5mo_check_manufacturer_id(struct i2c_client *client)
{
	int i, ret_val;
	u8 id;
	...
	for (i = 0; i < 3; i++) {
		ret_val = m5moLS_write_mem(client, 0x06, 2, addr[i], val[i]);
        if(ret_val < 0)
        {
	...
	ret_val = m5moLS_read_mem(client, sizeof(id), (u32)0x10000001, &id);
    if(ret_val < 0)
    {
        m5moLS_err(&client->dev, "return failed");
        return -1;
	...
	ret_val = m5moLS_write_mem(client, 0x06, sizeof(reset), (u32)0x10000000, reset);
    if(ret_val < 0)
    {
        m5moLS_err(&client->dev, "return failed");
        return -1;
	...
	m5moLS_info(&client->dev, "%#x\n", id);

	return id;
}

static int m5mo_program_fw(struct i2c_client *client, u8 *buf, u32 addr, u32 unit, u32 count, u8 id)
{
	u32 val;
	u32 intram_unit = 0x1000;
	int i, j, retries, ret_val = 0;
	int erase = 0x01;
	...
	for (i = 0; i < count; i++) {
		...
		ret_val = m5moLS_write_4byte(M5MO_CATEGORY_FLASH, M5MO_FLASH_ADDR, addr);
        if(ret_val < 0)
        {
            m5moLS_err(&client->dev, "return failed");
            return -1;
		...
		ret_val = m5moLS_write_1byte(M5MO_CATEGORY_FLASH, M5MO_FLASH_ERASE, erase);
        if(ret_val < 0)
        {
            m5moLS_err(&client->dev, "return failed");
            return -1;
		...
		do {
			mdelay(10);
			ret_val = m5moLS_read_1byte(M5MO_CATEGORY_FLASH, M5MO_FLASH_ERASE, &val);			
            if(ret_val < 0)
            {
                m5moLS_err(&client->dev, "return failed");
                return -1;
            }
		} while (val == erase && retries++ < M5MO_LS_I2C_VERIFY_RETRY);
		
		/* Set FLASH ROM programming size */
		ret_val = m5moLS_write_2byte(M5MO_CATEGORY_FLASH, M5MO_FLASH_BYTE, 
									unit == SZ_64K ? 0 : unit);
        if(ret_val < 0)
        {
            m5moLS_err(&client->dev, "return failed");
		...
		ret_val = m5moLS_write_1byte(M5MO_CATEGORY_FLASH, M5MO_FLASH_RAM_CLEAR, 0x01);
        if(ret_val < 0)
        {
            m5moLS_err(&client->dev, "return failed");
            return -1;
		...
		for (j = 0; j < unit; j += intram_unit) {
			ret_val = m5moLS_write_mem(client, 0x04, intram_unit, M5MO_INT_RAM_BASE_ADDR + j, buf + (i * unit) + j);
            if(ret_val < 0)
            {
                m5moLS_err(&client->dev, "return failed");
                return -1;
		...
		/* Start Programming */
		ret_val = m5moLS_write_1byte(M5MO_CATEGORY_FLASH, M5MO_FLASH_WR, 0x01);
        if(ret_val < 0)
        {
            m5moLS_err(&client->dev, "return failed");
            return -1;
		...
		do {
			mdelay(10);
			ret_val = m5moLS_read_1byte(M5MO_CATEGORY_FLASH, M5MO_FLASH_WR, &val);			
            if(ret_val < 0)
            {
                m5moLS_err(&client->dev, "return failed");
                return -1;
            }
		} while (val && retries++ < M5MO_LS_I2C_VERIFY_RETRY);
...
static int m5moLS_i2c_update_firmware(struct v4l2_subdev *sd)
{
    ...
    /* Set M-5MoLS Pins */
    ret_val = m5moLS_write_mem(client, 0x04, 0x0001, (u32)0x50000308, &pin1);
    if(ret_val < 0)
    {
        m5moLS_err(&client->dev, "return failed");
        return -1;
	...
	if(id < 0) 
	{
		m5moLS_err(&client->dev, "i2c falied, err %d\n", id);
		return id;
	}

	/* select flash memory */
    /* M-5MoLS flash memory select */   
    ret_val = m5moLS_write_1byte(M5MO_CATEGORY_FLASH, M5MO_FLASH_SEL,  (id == 0x01) ? 0x00 : 0x01);
    if(ret_val < 0)
    {
        m5moLS_err(&client->dev, "return failed");
        return -1;
    ...
    m5moLS_err(&client->dev, "Firmware Update Start!!(Manufacturer ID(%d))\n", id);
    m5moLS_write_fw_status(&status);

    buf = (u8 *)state->fw_info.addr;
    
	/* program FLSH ROM */
	ret_val = m5mo_program_fw(client, buf, M5MO_FLASH_BASE_ADDR, SZ_64K, 31, id);
    if(ret_val < 0)
    {
        m5moLS_err(&client->dev, "return failed");
        return -1;
	...
	if(id == 0x01) 
	{
		ret_val = m5mo_program_fw(client, buf, M5MO_FLASH_BASE_ADDR + SZ_64K * 31, SZ_8K, 4, id);
	} 
	else 
	{
		ret_val = m5mo_program_fw(client, buf, M5MO_FLASH_BASE_ADDR + SZ_64K * 31, SZ_4K, 8, id);
	}
    ...
    m5moLS_err(&client->dev, "Firmware Update Success!!\n");
...
static int m5moLS_detect(struct v4l2_subdev *sd)
{
    ...
    err = m5moLS_wait_interrupt(sd, M5MO_LS_INT_MODE, 2000);
    if(err)
    {
        m5moLS_err(&client->dev, "%s: m5moLS_wait_interrupt error!\n", __FUNCTION__);	
        return 0;
