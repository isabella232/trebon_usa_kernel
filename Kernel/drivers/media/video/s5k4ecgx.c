...
#define S5K4ECGX_AF_SEARCH_STATUS_IDLE	0x0000
#define S5K4ECGX_AF_SEARCH_STATUS_IN_PROGRESS	0x0001
#define S5K4ECGX_AF_SEARCH_STATUS_SUCCESS	0x0002
...
enum s5k4ecgx_capture_frame_size {
	...
	S5K4ECGX_CAPTURE_960_720,		/* 960x720 */
...
static const struct s5k4ecgx_framesize s5k4ecgx_capture_framesize_list[] = {
	...
	{ S5K4ECGX_CAPTURE_960_720,	960,  720 },
...
static const struct s5k4ecgx_regs regs_for_fw_version_1_1 = {
	...
	.capture_size = {
		...
		S5K4ECGX_REGSET(S5K4ECGX_CAPTURE_960_720, s5k4ecgx_960_720_Capture), 
...
#endif
	if(state->zoom_level  != 0 && state->framesize_index != S5K4ECGX_CAPTURE_WVGA &&
		state->framesize_index != S5K4ECGX_CAPTURE_VGA){
		if(state->zoom_ratio == 200){
				err = s5k4ecgx_set_from_table(sd, "capture_size",
							state->regs->capture_size,
							ARRAY_SIZE(state->regs->capture_size),
							S5K4ECGX_CAPTURE_1MP);
				if (err < 0) {
					dev_err(&client->dev,
						"%s: failed: i2c_write for capture_resolution\n",
						__func__);
				}		
		}else if(state->zoom_ratio == 160){
			err = s5k4ecgx_set_from_table(sd, "capture_size",
						state->regs->capture_size,
						ARRAY_SIZE(state->regs->capture_size),
						S5K4ECGX_CAPTURE_W1MP);
			if (err < 0) {
				dev_err(&client->dev,
					"%s: failed: i2c_write for capture_resolution\n",
					__func__);
					...
					struct v4l2_control *ctrl)
{
        ...
        if(!bStartFineSearch){
            ...
            if(AF_status == S5K4ECGX_AF_SEARCH_STATUS_IN_PROGRESS){   // Check if AF is in progress
                ctrl->value = CAMERA_AF_STATUS_IN_PROGRESS;
            }else{
                if(AF_status ==S5K4ECGX_AF_SEARCH_STATUS_SUCCESS){
...
*/
	s5k4ecgx_set_from_table(sd, "get ae stable status",
			&state->regs->get_ae_stable_status, 1, 0);
	s5k4ecgx_i2c_read_twobyte(client, 0x0F12, &AE_stable);
	pr_debug( "%s: ae stable status = %#x\n",
		__func__, AE_stable);
...
/*
	case V4L2_CID_CAMERA_AE_STABLE_RESULT:
		err = s5k4ecgx_get_ae_stable_status(sd, ctrl);
		break;
*/
