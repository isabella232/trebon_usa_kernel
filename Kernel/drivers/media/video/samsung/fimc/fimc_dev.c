...
#elif defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_TIKAL_MPCS)
...
#endif
    {
    	err = gpio_request(GPIO_CAM_FLASH_SET_NEW, "GPJ0(5)");		//GPIO_CAM_FLASH_SET
    	if (err) 
    	{
    		 printk(KERN_ERR "failed to request GPJ0(5) for camera control\n");
    		 return err;
    	}
...
ssize_t fimc_write(struct file *filp, const char *b, size_t c, loff_t *offset)
{
...
#endif
    {
    	...
    	{
    		 //gpio on
    		 gpio_direction_output(GPIO_CAM_FLASH_SET_NEW, 1);
    		 udelay(1);
    		 //gpio off
    		 gpio_direction_output(GPIO_CAM_FLASH_SET_NEW, 0);
    		 udelay(1);
