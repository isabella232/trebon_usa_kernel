...
static int fimc_camera_start(struct fimc_control *ctrl)
{
	...
	if (ret < 0) {
		...
		if (vtmode == 1 && (ctrl->cap->rotate == 90 || ctrl->cap->rotate == 270)){
                    ctrl->cam->window.left = 144;
    ...
    // cmk 2011.03.19 apply capture mode to fimc layer.
    if(ctrl->capture_mode == V4L2_MODE_PREVIEW)
    {
...
int fimc_s_parm(struct file *file, void *fh, struct v4l2_streamparm *a)
{
	...
	ctrl->capture_mode = a->parm.capture.capturemode; //Arun SISO add sensor state
...
int fimc_s_input(struct file *file, void *fh, unsigned int i)
{
	...
	fimc_dbg("%s: index %d\n", __func__, i);
#if defined(CONFIG_MACH_AEGIS) || defined(CONFIG_MARCH_VIPER)
	if (i == 1000) {
...
int fimc_s_ctrl_capture(void *fh, struct v4l2_control *c)
{
	...
	switch (c->id) {
       ...
       case V4L2_CID_CAMERA_POWER_CONTROL:
            mutex_unlock(&ctrl->v4l2_lock);
                ctrl->cam->cam_power(c->value);
                ctrl->cam->cam_power_flag = c->value;
            mutex_lock(&ctrl->v4l2_lock);   
            break;
#endif 
...
int fimc_streamon_capture(void *fh)
{
    ...
    printk("%s: camera sensor state: %d (0: preview, 1: capture) color : %d\n", __func__, ctrl->capture_mode,ctrl->cap->fmt.colorspace);

#if 1
	if(ctrl->capture_mode == V4L2_MODE_CAPTURE && ctrl->id != 2)
#else
	...
	// cmk 2011.04.18 fix a camera error when taking a self shot.
	// Camera capture ctrl should be set before starting to capture.
	if (ctrl->cap->fmt.colorspace != V4L2_COLORSPACE_JPEG){
		fimc_start_capture(ctrl);
	}
...
int fimc_streamoff_capture(void *fh)
{
	...
	// cmk 2011.03.23 SOC Only : Add capture done command to control flash.
	if(ctrl->capture_mode == V4L2_MODE_CAPTURE && ctrl->id != 2)
	{
		struct v4l2_control cam_ctrl;
	    int ret;

		cam_ctrl.id = V4L2_CID_CAM_CAPTURE_DONE;
		ret = subdev_call(ctrl, core, s_ctrl, &cam_ctrl);
		if(ret < 0 && ret != -ENOIOCTLCMD){
		    mutex_unlock(&ctrl->v4l2_lock);
			fimc_err("%s: Error in V4L2_CID_CAM_CAPTURE_DONE\n", __func__);
			return -EPERM;
		}
	}
	// end cmk
