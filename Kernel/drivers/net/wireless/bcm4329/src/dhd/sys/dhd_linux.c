...
uint dhd_roam = 0;//Enable AP Roaming
#else
uint dhd_roam = 1;//Disable AP Roaming
#endif
/* Control radio state */
uint dhd_radio_up = 1;
...
#endif
#ifndef CONFIG_TIKAL_MPCS		// Not to miss BC /MC , default dtim count value (= 1) is used.
...
dhd_os_wd_timer(void *bus, uint wdtick)
{
	...
	if (wdtick) {
		...
		if (save_dhd_watchdog_ms != dhd_watchdog_ms) {
			...
			*/
			//Error dection for Timer crash Issue problem in dhd watchdog timer
			init_timer(&dhd->timer);
