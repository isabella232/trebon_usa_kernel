...
#ifdef SOFTAP_SEL_80211_MODE		//Shinuk
static int wl_iw_set_80211_mode(struct net_device *dev, struct ap_profile *ap);
#endif
...
wl_iw_iscan_prep(wl_scan_params_t *params, wlc_ssid_t *ssid)
{
	...
	if (g_first_broadcast_scan == BROADCAST_SCAN_FIRST_STARTED) {
		if(g_init_scan_chan_num > 14 ){   //Prevent Memory Corruption 
...
wl_iw_iscan_set_scan_broadcast_prep(struct net_device *dev, uint flag)
{
	...
	if (g_first_broadcast_scan == BROADCAST_SCAN_FIRST_IDLE) {
		...
		if(g_init_scan_chan_num > 14 ) {    //Prevent Memory Corruption 
			WL_ERROR(("%s: g_init_scan_chan_num %d  is over than 14\n", __FUNCTION__, g_init_scan_chan_num));
...
set_ap_cfg(struct net_device *dev, struct ap_profile *ap)
{
	...
	if (ap_cfg_running == FALSE) {
        ...
        /* WMM disable , ARP offloading disable */
...
#ifdef SOFTAP_SEL_80211_MODE		//Shinuk
		/* set 802.11 mode with which SoftAP operates  */
		if ((res = wl_iw_set_80211_mode(ap_net_dev, &my_ap)) < 0) {
			WL_SOFTAP(("%s fail to set 80211 mode : %d\n", __FUNCTION__, res));
			goto fail;
		}
#endif
...
static int wl_iw_set_80211_mode(struct net_device *dev, struct ap_profile *ap)
{
	int res = 0;
	int nmode=1;
	int gmode=GMODE_AUTO;
	int isup=0;

	WL_ERROR(("%s: %s\n", __FUNCTION__, ap->op_mode));

	res |= dev_wlc_ioctl(dev, WLC_GET_UP, (void *)&isup, sizeof(int));

	/* nmode cmd should be done when wl is down */
	if(isup)
		res |= dev_wlc_ioctl(dev, WLC_DOWN, NULL, sizeof(int));

	if (ap->op_mode == 0) {
	   /* * ============  11 b/g/n default  =========== * */
           /* nmode = 1 , gmode = GMOD_AUTO */
		WL_SOFTAP(("%s: B/G/N MODE\n", __FUNCTION__));
	} else if (ap->op_mode == 1) {
	   /* * ============  11 b/g only  =========== * */
		nmode=0;
		gmode=GMODE_AUTO;
		WL_SOFTAP(("%s: B/g MODE\n", __FUNCTION__));
	} else if (ap->op_mode == 2) {
	   /* * ============  11 g only  =========== * */
		nmode=0;
		gmode=GMODE_ONLY;
		WL_SOFTAP(("%s: g only MODE\n", __FUNCTION__));
	} else {
	   /* * ============  11 b only  =========== * */
		nmode=0;
		gmode=GMODE_LEGACY_B;
		WL_SOFTAP(("%s: b only Mode\n", __FUNCTION__));
	}

	res |=dev_wlc_intvar_set(dev, "nmode", nmode);
	res |=dev_wlc_ioctl(dev, WLC_SET_GMODE, &gmode, sizeof(gmode));

	if(isup)
		res |= dev_wlc_ioctl(dev, WLC_UP, NULL, sizeof(int));
...
#ifdef SOFTAP_SEL_80211_MODE		//Shinuk
		/* set 802.11 mode with which SoftAP operates  */
	else if ((res = wl_iw_set_80211_mode(dev, &my_ap)) != 0) {
			WL_ERROR(("%s fail to set 80211 mode : %d\n", __FUNCTION__, res));
	}
#endif
	else {
#ifdef NEW_AP_INTERFACE
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	case WLC_E_DEAUTH:
#ifdef SOFTAP
	WL_TRACE(("STA left unexpectedly %d\n", event_type));
	if (ap_cfg_running) {
		wl_iw_send_priv_event(priv_dev, "STA_LEAVE");
		return;
	}
