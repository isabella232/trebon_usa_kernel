...
struct max8998_data {
	...
	struct mutex pmic_ldo_lock; // for suspend and resume lock
#endif	
...
#ifdef CONFIG_MACH_CHIEF
    mutex_init(&max8998->pmic_ldo_lock);
#endif
...
//LnT Temp Commented
//===================================================
//       BUCK1: VARM_1.2V_C110
//       BUCK2: VINT_1.1V_C110
//
// MAX8998_REG_ONOFF1: EN6: BUCK2 Enable/Disable
//                     EN7: BUCK1 Enable/Disable
//===================================================
        ret = max8998_read_reg(i2c, MAX8998_REG_ONOFF1, &on_off_1);
        if (ret) {
                        dev_err(max8998->dev, "Error reading MAX8998_REG_ONOFF4\n");
                        goto err_regulator;
        }
        on_off_1 = 0x3F & on_off_1;
        max8998_write_reg(i2c, MAX8998_REG_ONOFF1 ,on_off_1 );
        ret = max8998_read_reg(i2c, MAX8998_REG_ONOFF2, &on_off_2);
        if (ret) {
                        dev_err(max8998->dev, "Error reading MAX8998_REG_ONOFF2\n");
                        goto err_regulator;
        }
        on_off_2 = 0xF7 & on_off_2;
        max8998_write_reg(i2c, MAX8998_REG_ONOFF2 ,on_off_2);
//LnT Added:END----------------------------
...
#ifdef CONFIG_MACH_CHIEF
int max8998_ldo_enable_direct(int ldo,struct platform_device *pdev)
{
	...
	mutex_lock(&max8998->pmic_ldo_lock);

	if (ldo == MAX8998_LDO3) {
		max8998_read_reg(i2c, MAX8998_REG_ONOFF1,&value);
		
		value |= (1 << 2);
		max8998_write_reg(i2c,MAX8998_REG_ONOFF1,value);
	} else if (ldo == MAX8998_LDO8) {
		max8998_read_reg(i2c, MAX8998_REG_ONOFF2,&value);
		
		value |= (1 << 5);
		max8998_write_reg(i2c, MAX8998_REG_ONOFF2,value);
	} else if (ldo == MAX8998_LDO16) {
		max8998_read_reg(i2c, MAX8998_REG_ONOFF3,&value);
		
		value |= (1 << 5);
		max8998_write_reg(i2c, MAX8998_REG_ONOFF3,value);
	}


	mutex_unlock(&max8998->pmic_ldo_lock);

	return 0;
}

int max8998_ldo_disable_direct(int ldo,struct platform_device *pdev)
{
	int value, shift;
	struct max8998_data *max8998 = platform_get_drvdata(pdev);
    struct i2c_client *i2c = max8998->iodev->i2c;
...
static int max8998_suspend(struct platform_device *dev, pm_message_t state)
{
	/* Make sure for any other model LDO disable/enable is not done from here*/
	/*All LDO disable enable should be done from their respective driver suspend/resume*/
#ifndef CONFIG_TIKAL_MPCS 
    	printk("LNT DEBUG ----------- max8998 in suspend \n");	
		max8998_ldo_disable_direct(MAX8998_LDO3,dev);
    	max8998_ldo_disable_direct(MAX8998_LDO8,dev);
#endif
      /* This function will be called only for CHIEF*/		
	/* Fixed that an abnormal send_end interrupt happens after this funtion is called 
	  ( [SCH-R920] dongsik77.kim@samsunng.com, 2011.08.11. ) */
    	//max8998_ldo_disable_direct(MAX8998_LDO16,dev);

    	return 0;
}

static int max8998_resume(struct platform_device *dev)
{
	/* Make sure for any other model LDO disable/enable is not done from here*/
	/*All LDO disable enable should be done from their respective driver suspend/resume*/
#ifndef CONFIG_TIKAL_MPCS 
	   max8998_ldo_enable_direct(MAX8998_LDO3,dev);
       max8998_ldo_enable_direct(MAX8998_LDO8,dev);
#endif
      /* This function will be called only for CHIEF*/		
	/* Fixed that an abnormal send_end interrupt happens after max8998_suspend() funtion is called 
	  ( [SCH-R920] dongsik77.kim@samsunng.com, 2011.08.11. ) */
