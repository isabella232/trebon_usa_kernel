...
static ssize_t key_short_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int mask;
...
static DEVICE_ATTR(key, 0664, key_short_test_show, NULL);
#endif
...
static void gps_gpio_init(void)
{
	...
	key_dev = device_create(sec_class, NULL, 0, NULL, "key");
	if (IS_ERR(key_dev)) {
		pr_err("Failed to create device(gps)!\n");
		goto err;
	}
...
#define  S5PV210_VIDEO_SAMSUNG_MEMSIZE_FIMC0 (5538 * SZ_1K)	//buffer 8 -> 6
#define  S5PV210_VIDEO_SAMSUNG_MEMSIZE_FIMC1 (9900 * SZ_1K)
#define  S5PV210_VIDEO_SAMSUNG_MEMSIZE_FIMC2 (5538 * SZ_1K)	//buffer 8 -> 6
...
#define  S5PV210_VIDEO_SAMSUNG_MEMSIZE_JPEG (0 * SZ_1K)  //10024->0
#define  S5PV210_VIDEO_SAMSUNG_MEMSIZE_TEXTSTREAM (3000 * SZ_1K)
...
static struct i2c_gpio_platform_data i2c9_platdata = {
	.sda_pin		= FUEL_SDA_18V_REV02,
	.scl_pin		= FUEL_SCL_18V_REV02,
	.udelay 		= 2, /* 250KHz */
	.sda_is_open_drain	= 0,
	.scl_is_open_drain	= 0,
	.scl_is_output_only	= 0,
...
{
        if (HWREV < 2) {
                gpio_set_value(GPIO_MICBIAS_EN_REV01, wm8994_mic_bias);
                gpio_set_value(GPIO_MICBIAS_EN1_REV01, jack_mic_bias);
        } else {
...
static int isx005_regulator_init(void)
{
	...
	if (IS_ERR_OR_NULL(cam_3m_regulator)) {
		cam_3m_regulator = regulator_get(NULL, "cam_3m");	//LDO12
		if (IS_ERR_OR_NULL(cam_3m_regulator)) {
...
{
	/* CAM_MEGA_nRST - GPD0(0)*/
	if (gpio_request(GPIO_CAM_3M_nRST, "GPD0(0)") < 0){
		
		pr_err("failed gpio_request GPD0(0) for camera control\n");
	}
	/* GPIO_CAM_3M_nSTBY - GPJ1(2) */
	if (gpio_request(GPIO_CAM_3M_nSTBY, "GPE1(4)") < 0){
		
		pr_err("failed gpio_request GPE1(4) for camera control(%d)\n",__LINE__);
	}
}
...
static int isx005_ldo_en(bool en)
{
	...
	if (IS_ERR_OR_NULL(cam_io_regulator) ||		//LDO11
		...
		IS_ERR_OR_NULL(cam_3m_regulator)) {		//LDO12
...
static int isx005_power_on(void)
{
	...
	gpio_direction_output(GPIO_CAM_3M_nRST, 0);
	msleep(1);
	gpio_direction_output(GPIO_CAM_3M_nRST, 1);
	msleep(1);
	/* CAM_MEGA_nRST - GPD0(0) LOW */
	gpio_set_value(GPIO_CAM_3M_nRST, 1);
	mdelay(1);

	gpio_free(GPIO_CAM_3M_nRST);
	return 0;
}
...
static int isx005_power_off(void)
{
	...
	/* GPIO_CAM_3M_nSTBY - GPE1(4) LOW */
	gpio_direction_output(GPIO_CAM_3M_nSTBY, 1);
	gpio_set_value(GPIO_CAM_3M_nSTBY, 0);
	mdelay(3);


	/* CAM_MEGA_nRST - GPD0(0) LOW */
	gpio_direction_output(GPIO_CAM_3M_nRST, 1);
	gpio_set_value(GPIO_CAM_3M_nRST, 0);
	mdelay(1);
...
{
	/* GPIO_CAM_3M_nSTBY - GPE1(4) */
	if (gpio_request(GPIO_CAM_3M_nSTBY, "GPE1(4)") < 0){
		pr_err("failed gpio_request GPE1(4) for camera control(%d)\n",__LINE__);
	}
	mdelay(1);

	gpio_direction_output(GPIO_CAM_3M_nSTBY, 0);
	msleep(1);
	gpio_direction_output(GPIO_CAM_3M_nSTBY, 1);
	msleep(1);
	...
	if(en)
	{
		gpio_set_value(GPIO_CAM_3M_nSTBY, 1);
	} 
	else 
	{
		gpio_set_value(GPIO_CAM_3M_nSTBY, 0);
...
static struct sec_jack_buttons_zone sec_jack_buttons_zones[] = {
	...
	{
		/* 140 <= adc <= 320, stable zone */
		.code		= KEY_VOLUMEUP,
		.adc_low	= 140,
		.adc_high	= 320,
	},
	{
		/* 340 <= adc <= 350, stable zone */
		.code		= KEY_VOLUMEDOWN,
		.adc_low	= 340,
		.adc_high	= 350,
	},
	{
		/* 352 <= adc <= 355, stable zone */
		.code		= KEY_VOLUMEUP,
		.adc_low	= 352,
		.adc_high	= 355,
	},
	{
		/* 410 <= adc <= 580, stable zone */
		.code		= KEY_VOLUMEDOWN,
		.adc_low	= 410,
		.adc_high	= 580,
	},
	{
		/* 805 <= adc <= 807, stable zone */
		.code		= KEY_VOLUMEDOWN,
		.adc_low	= 804,
		.adc_high	= 810,
	},
...
static void config_init_gpio(void)
{
	...
	if (HWREV < 2)
		config_gpio_table(ARRAY_SIZE(initial_gpio_table_rev01), initial_gpio_table_rev01);
	else if ((HWREV >= 2) && (HWREV < 4))
		config_gpio_table(ARRAY_SIZE(initial_gpio_table_rev04), initial_gpio_table_rev04);
}
...
void config_sleep_gpio(void)
{
	...
	if (HWREV < 2)
		config_sleep_gpio_table(ARRAY_SIZE(sleep_gpio_table_rev01), sleep_gpio_table_rev01);
	else if ((HWREV >= 2) && (HWREV < 4))
		config_gpio_table(ARRAY_SIZE(sleep_alive_gpio_table_rev04), sleep_alive_gpio_table_rev04);

        if (HWREV < 2) {
                printk(KERN_DEBUG "SLPGPIO : BT(%d) WLAN(%d) UART_SEL(%d)\n",
                        gpio_get_value(GPIO_WLAN_BT_EN), gpio_get_value(GPIO_WLAN_EN), gpio_get_value(GPIO_UART_SEL));
                printk(KERN_DEBUG "SLPGPIO : CODEC_EN(%d) MIC_EN(%d) MIC_EN1(%d)\n",
                        gpio_get_value(GPIO_CODEC_LDO_EN), gpio_get_value(GPIO_MICBIAS_EN_REV01), gpio_get_value(GPIO_MICBIAS_EN1_REV01));
        } else {
                printk(KERN_DEBUG "SLPGPIO : BT(%d) WLAN(%d) UART_SEL(%d)\n",
                        gpio_get_value(GPIO_WLAN_BT_EN), gpio_get_value(GPIO_WLAN_EN_REV02), gpio_get_value(GPIO_UART_SEL));
                printk(KERN_DEBUG "SLPGPIO : CODEC_EN(%d) MIC_EN(%d) MIC_EN1(%d)\n",
...
static unsigned int wlan_on_gpio_table_REV02[][4] = {
	      {GPIO_WLAN_EN_REV02 , GPIO_WLAN_EN_AF, GPIO_LEVEL_HIGH, S3C_GPIO_PULL_NONE},
        {GPIO_WLAN_HOST_WAKE, GPIO_WLAN_HOST_WAKE_AF, GPIO_LEVEL_NONE, S3C_GPIO_PULL_DOWN},
};
...
static unsigned int wlan_off_gpio_table_REV02[][4] = {
		    {GPIO_WLAN_EN_REV02 , GPIO_WLAN_EN_AF, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},
        {GPIO_WLAN_HOST_WAKE, 0 , GPIO_LEVEL_NONE, S3C_GPIO_PULL_DOWN},
};
...
static int wlan_power_en(int onoff)
{
	...
	if(HWREV<2){
		s3c_gpio_cfgpin(GPIO_WLAN_EN,
				S3C_GPIO_SFN(GPIO_WLAN_EN_AF));
		...
		s3c_gpio_slp_setpull_updown(GPIO_WLAN_EN, S3C_GPIO_PULL_NONE);
	}else{
		s3c_gpio_cfgpin(GPIO_WLAN_EN_REV02,
				S3C_GPIO_SFN(GPIO_WLAN_EN_AF));
		s3c_gpio_setpull(GPIO_WLAN_EN_REV02, S3C_GPIO_PULL_NONE);
		gpio_set_value(GPIO_WLAN_EN_REV02, GPIO_LEVEL_HIGH);
		s3c_gpio_slp_cfgpin(GPIO_WLAN_EN_REV02, S3C_GPIO_SLP_OUT1);
		s3c_gpio_slp_setpull_updown(GPIO_WLAN_EN_REV02, S3C_GPIO_PULL_NONE);
	}
		...
		gpio_set_value(GPIO_WLAN_EN_REV02, GPIO_LEVEL_LOW);
		s3c_gpio_slp_cfgpin(GPIO_WLAN_EN_REV02, S3C_GPIO_SLP_OUT0);
		s3c_gpio_slp_setpull_updown(GPIO_WLAN_EN_REV02, S3C_GPIO_PULL_NONE);
	}
...
void wlan_setup_power(int on, int flag)
	{
		...
		if (on) {
			...
			s3c_config_gpio_alive_table(ARRAY_SIZE(wlan_on_gpio_table_REV02), wlan_on_gpio_table_REV02);
		}
			...
			if(HWREV<02){
				...
				s3c_config_gpio_alive_table(ARRAY_SIZE(wlan_off_gpio_table_REV02), wlan_off_gpio_table_REV02);
			}
...
#if defined(CONFIG_FB_S3C_TL2796) || defined(CONFIG_FB_S3C_S6D05A1X12)
	&s3c_device_spi_gpio,
#endif
	&sec_device_jack,
...
static void __init sound_init(void)
{
	...
	__raw_writel(reg, S5P_CLK_OUT);
        ...
        if (HWREV < 2) {
                gpio_request(GPIO_MICBIAS_EN_REV01, "micbias_enable");
                gpio_request(GPIO_MICBIAS_EN1_REV01, "sub_micbias_enable");
        } else {
...
static void __init aries_machine_init(void)
{
	...
	/* Replace I2C GPIO for old hwrev */
	if (HWREV < 0x2) {
		i2c9_platdata.scl_pin = FUEL_SCL_18V;
...
//	gp2a_gpio_init();
	...
	/* Replace I2C GPIO for old hwrev */
	if (HWREV < 0x4) {
		fimc_plat_lsi.camera[0]->i2c_busnum = 17;
		i2c17_platdata.scl_pin = GPIO_CAM_SCL_18V_REV01;
		i2c17_platdata.sda_pin = GPIO_CAM_SDA_18V_REV01;
	}
...
/* soonyong.cho : This is for setting unique serial number */
	...
	{
		if (gpio_is_valid(GPIO_MSENSE_nRST_REV02)) {
			if (gpio_request(GPIO_MSENSE_nRST_REV02, "GPA1(3)"))
				printk(KERN_ERR "Failed to request GPIO_MSENSE_nRST!\n");
			gpio_direction_output(GPIO_MSENSE_nRST_REV02, 1);
		}
		gpio_free(GPIO_MSENSE_nRST_REV02);
	}
}
...
void motor_control(bool onOff)
{
	...
	if (IS_ERR(regulator)) {
		printk(KERN_ERR "vmotor_regulator_get fail\n");
		return;
    }
