/* linux/arch/arm/mach-s5pv210/mach-tikal_uscc.c
 *
...
#include <mach/gpio.h>
#include <mach/gpio-tikal_uscc-settings.h>
#include <mach/adc.h>
...
#if !defined(CONFIG_GPS_GSD4T_USE)
...
#endif
	.temp_high_event_threshold	= 242,
	.temp_high_threshold	= 241,
	.temp_high_recovery	= 221,
	.temp_low_recovery	= 162,
	.temp_low_threshold	= 156,
	.temp_high_threshold_lpm	= 241,
	.temp_high_recovery_lpm		= 221,
	.temp_low_recovery_lpm		= 162,
	.temp_low_threshold_lpm		= 156,
...
static struct max8998_platform_data max8998_pdata = {
	...
	.buck1_voltage_set	= { 1275000, 1225000, 1050000, 950000 },
...
static int m5moLS_regulator_init(void)
{
    ...
    if(HWREV < 11)
    {
        if(IS_ERR_OR_NULL(cam_sensor_io_regulator)) 
        {
        	cam_sensor_io_regulator = regulator_get(NULL, "cam_sensor_io");
        	if(IS_ERR_OR_NULL(cam_sensor_io_regulator)) 
        	{
        		pr_err("failed to get cam_sensor_io regulator");
        		return -EINVAL;
        	}
        }
    }        
...
static void m5moLS_regulator_off(void)
{
    ...
    if(HWREV < 11)
    {
        if(camera_regulator_is_enabled(cam_sensor_io_regulator))
        {       
            err = regulator_disable(cam_sensor_io_regulator);
            if(err < 0) 
            {
                pr_err("Failed to disable regulator cam_sensor_io_regulator\n");
            }
        }
    }
...
static int m5moLS_power_on(void)
{
    ...
    /* CAM_VT_EN   */
    err = gpio_request(GPIO_CAM_VT_EN_28V, "GPJ4[1]");
    if(err < 0) 
    {
    ...
    /* GPIO_CAM_VT_RST_28V */
    err = gpio_request(GPIO_CAM_VT_RST_28V, "GPJ4[4]");
    if(err < 0) 
    {
        printk("failed to request GPJ1 for camera control\n");
    ...
    err = gpio_request(GPIO_CAM_IO_EN, "GPB[7]");
    if(err < 0)
    {
    ...
    if(HWREV < 11)
    {
        ...
        err = regulator_enable(cam_sensor_io_regulator);
        if(err < 0) 
        {
            pr_err("Failed to enable regulator cam_sensor_io_regulator\n");
            goto off;
        }    
    }
    else
...
static int sr130pc10_power_off()
{
    ...
    /* CAM_SENSOR_A2.8V */
    if(gpio_request(GPIO_CAM_IO_EN, "GPB[7]") != 0)
    {
        printk(KERN_ERR "failed to request GPB7 for camera control\n");
...
#endif
	&s3c_device_i2c9,  /* max1704x:fuel_guage, smb328a:switching_charger */
...
/* soonyong.cho : This is for setting unique serial number */
    ...
    if (HWREV < 0x12) {
        ...
        if (gpio_is_valid(GPIO_MSENSE_nRST_REV01)) {
            if (gpio_request(GPIO_MSENSE_nRST_REV01, "GPB(6)"))
                printk(KERN_ERR "Failed to request GPIO_MSENSE_nRST!\n");
            gpio_direction_output(GPIO_MSENSE_nRST_REV01, 1);
        }
        gpio_free(GPIO_MSENSE_nRST_REV01);
    }
}
...
void otg_phy_init(void)
{
	...
	/* squelch threshold tune [13:11] (001 : -15%) */
	__raw_writel((__raw_readl(S3C_USBOTG_PHYTUNE)
				& ~(0x3800)) | (0x3000), S3C_USBOTG_PHYTUNE);    
}
