...
struct gain_info_t playback_gain_table[PLAYBACK_GAIN_NUM] = {
	{ /* COMMON */
		...
		.gain = WM8994_HPOUT1_VU | 0x37		/* -10dB */
	}, {
		.mode = PLAYBACK_HP,
		...
		.reg  = WM8994_LEFT_OPGA_VOLUME,	/* 20h */
		.mask = WM8994_MIXOUTL_VOL_MASK,
		.gain = WM8994_MIXOUT_VU | 0x32         /* -7dB */
	}, {
		.mode = PLAYBACK_VOIP_RCV,
		.reg  = WM8994_RIGHT_OPGA_VOLUME,	/* 21h */
		.mask = WM8994_MIXOUTR_VOL_MASK,
		...
		.reg  = WM8994_SPEAKER_VOLUME_LEFT,	/* 26h */
		.mask = WM8994_SPKOUTL_VOL_MASK,
		.gain = WM8994_SPKOUT_VU | 0x35         /* -4dB */
	}, {
		.mode = PLAYBACK_VOIP_SPK,
		.reg  = WM8994_SPEAKER_VOLUME_RIGHT,	/* 27h */
		.mask = WM8994_SPKOUTR_VOL_MASK,
		...
		.reg  = WM8994_LEFT_OUTPUT_VOLUME,	/* 1Ch */
		.mask = WM8994_HPOUT1L_VOL_MASK,
		.gain = WM8994_HPOUT1_VU | 0x25		/* -20dB */
	}, {
		.mode = PLAYBACK_VOIP_HP,
		.reg  = WM8994_RIGHT_OUTPUT_VOLUME,	/* 1Dh */
		.mask = WM8994_HPOUT1R_VOL_MASK,
	...
	{ /* VOLTE_RCV */
		...
		.reg  = WM8994_LEFT_OPGA_VOLUME,	/* 20h */
		.mask = WM8994_MIXOUTL_VOL_MASK,
		.gain = WM8994_MIXOUT_VU | 0x3D         /* +4dB */
	}, {
		.mode = PLAYBACK_VOLTE_RCV,
		.reg  = WM8994_RIGHT_OPGA_VOLUME,	/* 21h */
		.mask = WM8994_MIXOUTR_VOL_MASK,
...
struct gain_info_t voicecall_gain_table[VOICECALL_GAIN_NUM] = {
	{ /* COMMON */
		...
		.reg  = WM8994_CLASSD,			/* 25h */
		.mask = WM8994_SPKOUTL_BOOST_MASK,
		.gain = 0x7 << WM8994_SPKOUTL_BOOST_SHIFT	/* +9dB */
	}, { /* HP */
		.mode = VOICECALL_HP,
		...
		.reg  = WM8994_LEFT_OUTPUT_VOLUME,	/* 1Ch */
		.mask = WM8994_HPOUT1L_VOL_MASK,
		.gain = WM8994_HPOUT1_VU | 0x2D		/* -12dB */
	}, {
		.mode = VOICECALL_HP_NO_MIC,
		.reg  = WM8994_RIGHT_OUTPUT_VOLUME,	/* 1Dh */
		.mask = WM8994_HPOUT1R_VOL_MASK,
...
struct gain_info_t recording_gain_table[RECORDING_GAIN_NUM] = {
	{ /* MAIN */
		...
		.reg  = WM8994_LEFT_LINE_INPUT_1_2_VOLUME,	/* 18h */
		.mask = WM8994_IN1L_VOL_MASK,
		.gain = WM8994_IN1L_VU | 0x1F		/* +30.0dB */
	}, {
		.mode = RECORDING_MAIN,
		.reg  = WM8994_INPUT_MIXER_3,		/* 29h */
		.mask = WM8994_IN1L_MIXINL_VOL_MASK | WM8994_MIXOUTL_MIXINL_VOL_MASK,
	...
	{ /* VOLTE_MAIN */
		...
		.mode = RECORDING_VOLTE_MAIN2,
		.reg  = WM8994_RIGHT_LINE_INPUT_3_4_VOLUME,	/* 1Bh */
		.mask = WM8994_IN2R_VOL_MASK,
		.gain = WM8994_IN2R_VU | 0x0F		/* +6dB */
	}, {
		.mode = RECORDING_VOLTE_MAIN2,
		.reg  = WM8994_INPUT_MIXER_4,		/* 2Ah */
		.mask = WM8994_IN2R_MIXINR_VOL_MASK | WM8994_MIXOUTR_MIXINR_VOL_MASK,
		...
		.mode = RECORDING_VOLTE_HP,
		.reg  = WM8994_RIGHT_LINE_INPUT_1_2_VOLUME,	/* 1Ah */
		.mask = WM8994_IN1R_VOL_MASK,
		.gain = WM8994_IN1R_VU | 0x6		/* -7.5dB */
	}, {
		.mode = RECORDING_VOLTE_HP,
		.reg  = WM8994_INPUT_MIXER_4,		/* 2Ah */
		.mask = WM8994_IN1R_MIXINR_VOL_MASK | WM8994_MIXOUTR_MIXINR_VOL_MASK,
	...
	{ /* GTALK_MAIN */
		...
		.mode = RECORDING_GTALK_MAIN2,
		.reg  = WM8994_RIGHT_LINE_INPUT_3_4_VOLUME,	/* 1Bh */
		.mask = WM8994_IN2R_VOL_MASK,
		.gain = WM8994_IN2R_VU | 0x1D		/* +27dB */
	}, {
		.mode = RECORDING_GTALK_MAIN2,
		.reg  = WM8994_INPUT_MIXER_4,		/* 2Ah */
		.mask = WM8994_IN2R_MIXINR_VOL_MASK | WM8994_MIXOUTR_MIXINR_VOL_MASK,
...
void audio_ctrl_mic_bias_gpio(struct wm8994_platform_data *pdata, int enable)
{
	...
	if (!pdata)
		...
		if (enable == 1) { 
			pdata->set_mic_bias(0, true);  // main_micbias on
			pdata->set_mic_bias(1, false); // sub_micbias off
		}
		else if (enable == 2) { 
			pdata->set_mic_bias(0, false); // main_micbias off
			pdata->set_mic_bias(1, true);  // sub_micbias on
		}
		else {
			pdata->set_mic_bias(0, false); // main_micbias off
			pdata->set_mic_bias(1, false); // sub_micbias off
	...
	if(!(wm8994->codec_state & CALL_ACTIVE)) {
		...
		wm8994_write(codec, WM8994_DAC2_RIGHT_MIXER_ROUTING, val);
                ...
                if (wm8994->gtalk_active_flag)
                        wm8994_set_codec_gain(codec, RECORDING_MODE, RECORDING_GTALK_HP);
		else
#endif
#ifdef CONFIG_VOLTE
                if (wm8994->volte_active_flag)
                        wm8994_set_codec_gain(codec, RECORDING_MODE, RECORDING_VOLTE_HP);
		else
#endif
	...
	if(!(wm8994->codec_state & CALL_ACTIVE)) {
		...
		wm8994_write(codec, WM8994_DAC2_RIGHT_MIXER_ROUTING, val);
	  		...
	  		wm8994_set_codec_gain(codec, RECORDING_MODE,
					RECORDING_GTALK_MAIN);
                else
#endif
#ifdef CONFIG_VOLTE
                if (wm8994->volte_active_flag)
	  		wm8994_set_codec_gain(codec, RECORDING_MODE,
					RECORDING_VOLTE_MAIN);
                else
#endif
		if ((wm8994->input_source == VOIP_INPUT)
#ifdef CONFIG_VOIP_EXT
...
void wm8994_record_2nd_main_mic(struct snd_soc_codec *codec)
{
	...
	audio_ctrl_mic_bias_gpio(wm8994->pdata, 2);
	...
	if(!(wm8994->codec_state & CALL_ACTIVE)) {
		...
		wm8994_write(codec, WM8994_DAC2_RIGHT_MIXER_ROUTING, val);
                ...
                if (wm8994->gtalk_active_flag)
			wm8994_set_codec_gain(codec, RECORDING_MODE,
					RECORDING_GTALK_MAIN2);
		else
#endif
#ifdef CONFIG_VOLTE
                if (wm8994->volte_active_flag)
			wm8994_set_codec_gain(codec, RECORDING_MODE,
					RECORDING_VOLTE_MAIN2);
		else
#endif
		if ((wm8994->input_source == VOIP_INPUT)
#ifdef CONFIG_VOIP_EXT
...
#ifdef CONFIG_VOIP_GTALK
        if (wm8994->gtalk_active_flag)
                wm8994_set_codec_gain(codec, PLAYBACK_MODE, PLAYBACK_GTALK_RCV);
        else
#endif
#ifdef CONFIG_VOLTE
        if (wm8994->volte_active_flag)
                wm8994_set_codec_gain(codec, PLAYBACK_MODE, PLAYBACK_VOLTE_RCV);
        else
#endif
...
#ifdef CONFIG_VOIP_GTALK
        else if (wm8994->gtalk_active_flag)
		wm8994_set_codec_gain(codec, PLAYBACK_MODE, PLAYBACK_GTALK_HP);
#endif
#ifdef CONFIG_VOLTE
        else if (wm8994->volte_active_flag)
		wm8994_set_codec_gain(codec, PLAYBACK_MODE, PLAYBACK_VOLTE_HP);
#endif
...
void wm8994_set_playback_speaker(struct snd_soc_codec *codec)
{
	...
	if (wm8994->output_source == RING_TONE) {
		...
		wm8994_set_codec_gain(codec, PLAYBACK_MODE, PLAYBACK_RING_SPK);
                ...
                wm8994_set_codec_gain(codec, PLAYBACK_MODE, PLAYBACK_GTALK_SPK);
#endif
#ifdef CONFIG_VOLTE
        else if (wm8994->volte_active_flag)
                wm8994_set_codec_gain(codec, PLAYBACK_MODE, PLAYBACK_VOLTE_SPK);
#endif
	else if ((wm8994->output_source == VOIP_OUTPUT)
#ifdef CONFIG_VOIP_EXT
                || (wm8994->voip_start_flag)
...
void wm8994_set_voicecall_receiver(struct snd_soc_codec *codec)
{
	...
	/* Sidetone */
	wm8994_write(codec, WM8994_DAC1_MIXER_VOLUMES, 0x0003);
...
void wm8994_set_voicecall_headset(struct snd_soc_codec *codec)
{
	...
	val = wm8994_read(codec, WM8994_LEFT_OPGA_VOLUME);
	val &= ~WM8994_MIXOUTL_MUTE_N_MASK;
	val |= WM8994_MIXOUT_VU | WM8994_MIXOUTL_MUTE_N | 0x39;
	wm8994_write(codec, WM8994_LEFT_OPGA_VOLUME, val);

	val = wm8994_read(codec, WM8994_RIGHT_OPGA_VOLUME);
	val &= ~WM8994_MIXOUTR_MUTE_N_MASK;
	val |= WM8994_MIXOUT_VU | WM8994_MIXOUTR_MUTE_N | 0x39;
	wm8994_write(codec, WM8994_RIGHT_OPGA_VOLUME, val);
	...
	if (loopback_mode == PBA_LOOPBACK_MODE_ON) {
		...
		val = wm8994_read(codec, WM8994_RIGHT_LINE_INPUT_1_2_VOLUME);
		val &= ~(WM8994_IN1R_VOL_MASK);
		val |= (WM8994_IN1R_VU | 0x1B); /* +24dB */
		wm8994_write(codec, WM8994_RIGHT_LINE_INPUT_1_2_VOLUME, val);

		wm8994_write(codec, WM8994_LEFT_OUTPUT_VOLUME, 0x179);
		wm8994_write(codec, WM8994_RIGHT_OUTPUT_VOLUME, 0x179);
	} else if (loopback_mode == SIMPLETEST_LOOPBACK_MODE_ON) {
		DEBUG_LOG("SIMPLETEST_LOOPBACK_MODE_ON");

		val = wm8994_read(codec, WM8994_RIGHT_LINE_INPUT_1_2_VOLUME);
		val &= ~(WM8994_IN1R_VOL_MASK);
		val |= (WM8994_IN1R_VU | 0x1C); /* +25.5dB */
		wm8994_write(codec, WM8994_RIGHT_LINE_INPUT_1_2_VOLUME, val);

		wm8994_write(codec, WM8994_LEFT_OUTPUT_VOLUME, 0x179); /* 0dB */
		wm8994_write(codec, WM8994_RIGHT_OUTPUT_VOLUME, 0x179); /* 0dB */
	} else 
#endif
	{
		wm8994_write(codec, WM8994_LEFT_OUTPUT_VOLUME, 0x172);
		wm8994_write(codec, WM8994_RIGHT_OUTPUT_VOLUME, 0x172);
	}
...
void wm8994_set_voicecall_speaker(struct snd_soc_codec *codec)
{
	...
	if (loopback_mode == SIMPLETEST_LOOPBACK_MODE_ON) {
		DEBUG_LOG("SIMPLETEST_LOOPBACK_MODE_ON");
		wm8994_write(codec, WM8994_RIGHT_LINE_INPUT_3_4_VOLUME, 0x109);
	}
#endif
...
int wm8994_set_codec_gain(struct snd_soc_codec *codec, u16 mode, u32 device)
{
	...
	if (mode == PLAYBACK_MODE) {
		...
		case PLAYBACK_VOIP_BT:
			...
			break;
#ifdef CONFIG_VOLTE
		case PLAYBACK_VOLTE_RCV:
			gain_set_bits |= PLAYBACK_VOLTE_RCV;
			break;
		case PLAYBACK_VOLTE_SPK:
			gain_set_bits |= PLAYBACK_VOLTE_SPK;
			break;
		case PLAYBACK_VOLTE_HP:
			gain_set_bits |= PLAYBACK_VOLTE_HP;
			break;
		case PLAYBACK_VOLTE_BT:
			gain_set_bits |= PLAYBACK_VOLTE_BT;
			break;
#endif
#ifdef CONFIG_VOIP_GTALK
		case PLAYBACK_GTALK_RCV:
			gain_set_bits |= PLAYBACK_GTALK_RCV;
			break;
		case PLAYBACK_GTALK_SPK:
			gain_set_bits |= PLAYBACK_GTALK_SPK;
			break;
		case PLAYBACK_GTALK_HP:
			gain_set_bits |= PLAYBACK_GTALK_HP;
			break;
		case PLAYBACK_GTALK_BT:
			gain_set_bits |= PLAYBACK_GTALK_BT;
			break;
#endif
		default:
		...
		case RECORDING_VOIP_BT:
			...
			break;
#ifdef CONFIG_VOLTE
		case RECORDING_VOLTE_MAIN:
			gain_set_bits |= RECORDING_VOLTE_MAIN;
			break;
		case RECORDING_VOLTE_MAIN2:
			gain_set_bits |= RECORDING_VOLTE_MAIN2;
			break;                        
		case RECORDING_VOLTE_HP:
			gain_set_bits |= RECORDING_VOLTE_HP;
			break;
		case RECORDING_VOLTE_BT:
			gain_set_bits |= RECORDING_VOLTE_BT;
			break;
#endif
#ifdef CONFIG_VOIP_GTALK
		case RECORDING_GTALK_MAIN:
			gain_set_bits |= RECORDING_GTALK_MAIN;
			break;
		case RECORDING_GTALK_MAIN2:
			gain_set_bits |= RECORDING_GTALK_MAIN2;
			break;                        
		case RECORDING_GTALK_HP:
			gain_set_bits |= RECORDING_GTALK_HP;
			break;
		case RECORDING_GTALK_BT:
			gain_set_bits |= RECORDING_GTALK_BT;
			break;
#endif
		default:
