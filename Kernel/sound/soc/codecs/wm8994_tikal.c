...
#ifdef CONFIG_SND_SOC_A1026
int wm8994_get_nc_active(struct snd_soc_codec *codec)
{
        struct wm8994_priv *wm8994 = codec->drvdata;
        int result;
...
#endif
	if (wm8994->ncbypass_active)
                result = 0;
	else
                result = 1;


	DEBUG_LOG("NC_bypass(%d) loopback(%d) result(%d)", 
                wm8994->ncbypass_active, loopback_mode, result);
...
#ifdef CONFIG_SND_SOC_A1026
        if(wm8994_get_nc_active(codec))
        	wm8994_set_codec_gain(codec, VOICECALL_MODE, VOICECALL_NC_RCV);
        else 
...
void wm8994_set_voicecall_headset(struct snd_soc_codec *codec)
{
	...
	if (loopback_mode == PBA_LOOPBACK_MODE_ON) {
		...
		val |= (WM8994_HPOUT1_VU | WM8994_HPOUT1L_MUTE_N | 0x37);
		wm8994_write(codec, WM8994_LEFT_OUTPUT_VOLUME, val);

		val = wm8994_read(codec, WM8994_RIGHT_OUTPUT_VOLUME);
		val &= ~(WM8994_HPOUT1R_MUTE_N_MASK | WM8994_HPOUT1R_VOL_MASK);
		val |= (WM8994_HPOUT1_VU | WM8994_HPOUT1R_MUTE_N | 0x37);
		wm8994_write(codec, WM8994_RIGHT_OUTPUT_VOLUME, val);
	}
#endif
...
#endif
#ifdef CONFIG_SND_SOC_A1026
		case VOICECALL_NC_RCV:
			gain_set_bits |= (VOICECALL_RCV | VOICECALL_NC_RCV);
			break;
#endif
		default:
