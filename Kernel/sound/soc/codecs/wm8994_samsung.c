...
static int proc_read_ttymode(char *page, char **start, off_t off, int count, 
							int *eof, void *data)
{
...
static int proc_write_ttymode(struct file *file, const char *buffer, 
...
int init_tty_mode_procfs(void)
{
	...
	/* everything OK */
	printk(KERN_INFO "TTY (%s) procfs initialized\n", TTY_DIR_NAME);
...
unsigned int pba_loopback_mode;
unsigned int simpletest_loopback_mode;

static struct proc_dir_entry *loopback_procfs_dir, *pba_loopback_mode_file, 
	*simpletest_loopback_mode_file;

static void set_loopback_mode(void)
{
        if (pba_loopback_mode)
                loopback_mode = PBA_LOOPBACK_MODE_ON;
        else if (simpletest_loopback_mode)
                loopback_mode = SIMPLETEST_LOOPBACK_MODE_ON;
        else
                loopback_mode = LOOPBACK_MODE_OFF;

	DEBUG_LOG("loopback_mode(%d)", loopback_mode);
}

static int proc_read_loopback_mode(char *page, char **start, off_t off, 
...
static int proc_write_pba_loopback_mode(struct file *file, const char *buffer, 
						unsigned long count, void *data)
{
	char buf[] = "0x00000000\n";
	unsigned long len = min((unsigned long)sizeof(buf) - 1, count);
	...
	if (sscanf(buf, "%li", &val) != 1)
		printk("%s is not in hex or decimal form.\n", buf); 
                ...
                pba_loopback_mode = val;

	DEBUG_LOG("pba_loopback_mode(%d)", pba_loopback_mode);
...
static int proc_write_simpletest_loopback_mode(struct file *file, 
		        const char *buffer, unsigned long count, void *data)
{
	...
	if (sscanf(buf, "%li", &val) != 1)
		printk(": %s is not in hex or decimal form.\n", buf); 
                ...
                simpletest_loopback_mode = val;

	DEBUG_LOG("simpletest_loopback_mode(%d)", simpletest_loopback_mode);
...
int init_loopback_mode_procfs(void)
{
	int ret = 0;

#ifdef FEATURE_TTY
	loopback_procfs_dir = tty_procfs_dir;
#else
	/* create directory */
	loopback_procfs_dir = proc_mkdir(LOOPBACK_DIR_NAME, NULL);
	if(loopback_procfs_dir == NULL) {
		ret = -ENOMEM;
		goto out;
	}
#endif
	...
	pba_loopback_mode_file = create_proc_entry("loopback_mode", 0666, 
							loopback_procfs_dir);
	simpletest_loopback_mode_file = create_proc_entry("loopback_15", 0666, 
							loopback_procfs_dir);

	if (!pba_loopback_mode_file || !simpletest_loopback_mode_file) {
		ret = -ENOMEM;
		goto no_loopback_mode;
	}
	...
	simpletest_loopback_mode_file->write_proc = 
                                        proc_write_simpletest_loopback_mode;

	/* everything OK */
	printk(KERN_INFO "Loopback (%s) procfs initialized\n", 
					        	LOOPBACK_DIR_NAME);

	return 0;

no_loopback_mode:
	remove_proc_entry("loopback_mode", loopback_procfs_dir);
	remove_proc_entry(LOOPBACK_DIR_NAME, NULL);
#ifdef FEATURE_TTY	
	return ret;
#else
out:
...
static const char *mic_path[] = {
	"Main Mic", "Main2 Mic", "Hands Free Mic", "BT Sco Mic", "MIC OFF"
};
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	case MAIN:
                if ((wm8994->cur_path == SPK)
                        && ((wm8994->codec_state & CALL_ACTIVE)
                                || wm8994_get_voip_status(codec)
                )) {
                        DEBUG_LOG("Set mic2 path in speaker call");
	...
	default:
        ...
        if (wm8994_get_voip_status(codec) &&
                (wm8994->cur_path >= RCV && wm8994->cur_path <= BT)) {
                wm8994_set_voip_path(codec, wm8994->cur_path, wm8994->rec_path);
                return 0;
        }
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	case BT:
                ...
                if (wm8994_get_voip_status(codec)) {
                        wm8994->codec_state |= PLAYBACK_ACTIVE;
                        wm8994_set_voip_path(codec, path_num, wm8994->rec_path);
                        return 0;
                }
                break;
	case SPK_HP:
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	case CMD_RECOGNITION_DEACTIVE:
		...
		wm8994->recognition_active = RECG_OFF;
		break;

	case CMD_RECOGNITION_ACTIVE:
		DEBUG_LOG("Recognition Gain is activated!!");
		wm8994->recognition_active = RECG_ON;
		break;

#ifdef CONFIG_SND_SOC_A1026
	/* To setting noise suppressor bypass */
	case CMD_NC_BYPASS_DEACTIVE:
		DEBUG_LOG("NC BYPASS is deactivated!!");                
		wm8994->ncbypass_active = NC_OFF;
		break;

	case CMD_NC_BYPASS_ACTIVE:
		DEBUG_LOG("NC BYPASS is activated!!");
		wm8994->ncbypass_active = NC_ON;
		break;
#endif

	case CMD_MUTE_CODEC_CALL_PATH:
...
#ifdef CONFIG_VOLTE
        /* To set codec register for VoLTE */
	case CMD_VOLTE_ACTIVE:
		DEBUG_LOG("Start VoLTE");
		wm8994->volte_active_flag = 1;
		break;

	case CMD_VOLTE_DEACTIVE:
		DEBUG_LOG("Stop VoLTE");
		wm8994->volte_active_flag = 0;
		break;
#endif

#ifdef CONFIG_VOIP_GTALK
        /* To set codec register for GTalk */
	case CMD_VOIP_GTALK_ACTIVE:
		DEBUG_LOG("Start GTalk");
		wm8994->gtalk_active_flag = 1;
		break;

	case CMD_VOIP_GTALK_DEACTIVE:
		DEBUG_LOG("Stop GTalk");
		wm8994->gtalk_active_flag = 0;
		break;
#endif

	/* To set codec register for AMR recording */
	case CMD_VOICE_RECORDER_AMR_ACTIVE:
		DEBUG_LOG("Start AMR recording");
		wm8994->amr_rec_flag = 1;
		break;

	case CMD_VOICE_RECORDER_AMR_DEACTIVE:
		DEBUG_LOG("Stop AMR recording");
		wm8994->amr_rec_flag = 0;
		break;
...
void wm8994_set_voip_gain(struct snd_soc_codec *codec, int play_path)
{
	struct wm8994_priv *wm8994 = codec->drvdata;
	u32 device = COMMON_SET_BIT;
	
#ifdef CONFIG_VOIP_GTALK
	if (wm8994->gtalk_active_flag) {
		if (play_path == RCV)
			device = PLAYBACK_GTALK_RCV;
		else if (play_path == SPK)
			device = PLAYBACK_GTALK_SPK;
		else if (play_path == HP || play_path == HP_NO_MIC)
			device = PLAYBACK_GTALK_HP;
	} else
#endif
#ifdef CONFIG_VOLTE
	if (wm8994->volte_active_flag) {
		if (play_path == RCV)
			device = PLAYBACK_VOLTE_RCV;
		else if (play_path == SPK)
			device = PLAYBACK_VOLTE_SPK;
		else if (play_path == HP || play_path == HP_NO_MIC)
			device = PLAYBACK_VOLTE_HP;
	} else
#endif
	if ((wm8994->output_source == VOIP_OUTPUT)
		...
		if (play_path == RCV)
			device = PLAYBACK_VOIP_RCV;
		else if (play_path == SPK)
			device = PLAYBACK_VOIP_SPK;
		else if (play_path == HP || play_path == HP_NO_MIC)
			device = PLAYBACK_VOIP_HP;
	} 

	wm8994_set_codec_gain(codec, PLAYBACK_MODE, device);
}

int wm8994_get_voip_status(struct snd_soc_codec *codec)
{
        struct wm8994_priv *wm8994 = codec->drvdata;
        int status = 0;
...
#endif
#ifdef CONFIG_VOLTE
                || wm8994->volte_active_flag
#endif
#ifdef CONFIG_VOIP_GTALK
                || wm8994->gtalk_active_flag
#endif
        ) {
                status = 1;
        }
...
void wm8994_set_voip_path(struct snd_soc_codec *codec, int play_path, int rec_path)
{
        struct wm8994_priv *wm8994 = codec->drvdata;
        int voip_rec_path;


        DEBUG_LOG("(%d)", play_path);

#ifdef CONFIG_DVFS_LIMIT
        if (!wm8994->voip_dvfs_lock_flag) {
                DEBUG_LOG("Set DVFS lock");
                s5pv210_lock_dvfs_high_level(DVFS_LOCK_TOKEN_3, L2);
                wm8994->voip_dvfs_lock_flag = 1;
        }
#endif

        if (wm8994->cur_path != play_path) {
		wm8994_disable_path(codec);
				
                wm8994->cur_path = play_path;
                wm8994->universal_playback_path[play_path] (codec);
        } else {
                int val;

                DEBUG_LOG("Skip to set playback path");

		if ((wm8994->output_source != RING_TONE) && (play_path != BT)) {
	                DEBUG_LOG("Set only playback gain for VoIP");
			wm8994_set_voip_gain(codec, play_path);
		}
        ...
        if (play_path == RCV || play_path == HP_NO_MIC)
                voip_rec_path = MAIN;
        else if (play_path == SPK)
                voip_rec_path = MAIN2;
        else if (play_path == HP)
                voip_rec_path = SUB;
        else if (play_path == BT)
                voip_rec_path = BT_REC;
        else 
                voip_rec_path = MIC_OFF;

        if (rec_path != MIC_OFF)
                wm8994->rec_path = voip_rec_path;

        wm8994->universal_mic_path[voip_rec_path] (codec);
}
...
#ifdef CONFIG_SND_SOC_A1026		
		|| ((wm8994->prev_ncbypass_active != wm8994->ncbypass_active) 
			&& (wm8994->cur_path == path_num)
		)
#endif
	) {
...
#ifdef CONFIG_DVFS_LIMIT
                        if (wm8994->voip_dvfs_lock_flag) {
                                DEBUG_LOG("Clear DVFS lock");
                                s5pv210_unlock_dvfs_high_level(DVFS_LOCK_TOKEN_3);
                                wm8994->voip_dvfs_lock_flag = 0;
                        }
#endif
                }
